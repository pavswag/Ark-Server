    ...
    public void tick(TickableContainer<Player> container, Player player) {
        ...
        if (successFormulaPredicate == null || successFormulaPredicate.test(this)) {
            ...
            if (itemsProduced != null) {
                Arrays.stream(itemsProduced).forEach(item -> {
                    if (item.getId() == 2) {
                        player.sendSpamMessage(" to form 4 cannonballs.");
                        player.sendSpamMessage("You remove the cannonballs from the mould");
                    }
            ...
            if (itemProductionFunction != null) {
                ...
                if (produced != null) {
                    Arrays.stream(produced).forEach(item -> {
                        if (item.getId() == 2) {
                            player.sendSpamMessage(" to form 4 cannonballs.");
                            player.sendSpamMessage("You remove the cannonballs from the mould");
                        }
