...
public class Clan {
	...
	public void updateInterface(Player player) {
		...
		for (int index = 0; index < 100; index++) {
			if (index < activeMembers.size()) {
				player.getPA().sendFrame126(getDRank(activeMembers.get(index).getLoginName()) + " <clan=" + getRank(activeMembers.get(index).getLoginName()) + ">" + activeMembers.get(index).getDisplayName(), 18144 + index);
			} else {
	...
	public void sendChat(Player paramClient, String paramString) {
		...
		getPlayers().forEachFiltered(c -> activeMembers.stream().anyMatch(it -> it.is(c)), c -> {
			if (!c.getFriendsList().getRepository().isIgnore(paramClient)) {
				c.sendMessage("/@bla@[@blu@" + getTitle() + "@bla@] " + paramClient.getDisplayNameFormatted() + ": @dre@"
						+ Misc.formatPlayerName(finalParamString.substring(1)));
			}
		});
	}
	 ...
	 * @return
	 */
	public String getDRank(String name) {

		for (Player player : getPlayers().toPlayerArray()) {
			if (player.getLoginName().equalsIgnoreCase(name) && !player.hideDonor) {
				return player.getRights().buildCrownString();
			}
		}
...
//		}
	...
	public void kickMember(String name) {
		...
		sendMessage("@blu@[Attempting to kick/ban @red@'" + Misc.formatPlayerName(name) + "'" + " @blu@from this friends chat]");
	}
