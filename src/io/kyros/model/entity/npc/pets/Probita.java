...
public enum Probita {
	...
	private final int invalidPet;
	private final int validPet;
	 ...
	 */
	final static int[] INVALID_PETS = { 15571, 15572, 8135, 15568 };

	public static void cancellationOfPreviousPet(final Player player) {
		if (IntStream.of(INVALID_PETS).anyMatch(id -> player.petSummonId == id)) {
			if (player.petSummonId == 15572) {
