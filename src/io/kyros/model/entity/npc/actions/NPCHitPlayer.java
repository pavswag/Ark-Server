...
import io.kyros.content.bosses.Scorpia;
...
import io.kyros.content.combat.effects.damageeffect.impl.armour.SerpentineHelmEffect;
import io.kyros.content.combat.formula.CombatFormula;
import io.kyros.content.combat.formula.MagicMaxHit;
import io.kyros.content.combat.formula.MeleeMaxHit;
import io.kyros.content.combat.formula.RangeMaxHit;
...
import io.kyros.content.combat.npc.NPCAutoAttackDamage;
...
public class NPCHitPlayer {
    ...
    public static CombatHit applyAutoAttackDamage(NPC npc, Player c, NPCAutoAttack npcAutoAttack) {
        ...
        int damage = (maxHit > 0 && npcAutoAttack.getMinHit() < maxHit)
                ? Misc.random(npcAutoAttack.getMinHit(), maxHit)
                : npcAutoAttack.getMinHit();
        ...
        if (npc.getNpcId() == 12818 || npc.getNpcId() == 12817 || npc.getNpcId() == 11775 || npc.getNpcId() == 12783 || npc.getNpcId() == 12821 ||
                npc.getNpcId() == 12812 || npc.getNpcId() == 12607 || npc.getNpcId() == 853 ||npc.getNpcId() == 2956 || npc.getNpcId() == 1801 || npc.getNpcId() == 12223 ||
                npc.getNpcId() == 5797 || npc.getNpcId() == 12617  || npc.getNpcId() == 11756 || autoAttackAccuracyBonus >= 75 || npc.getNpcId() == 10936 ||
                npc.getNpcId() == 12813) {
            accuracyRoll = 75.0;
        }

        if (npc.getNpcId() == 7649 && Boundary.isIn(npc, Boundary.CHAOTIC_ZONE)) {
            //TODO Chaotic Death Spawn 1 Shot Mechanic
            CombatType combatType = npcAutoAttack.getCombatType();
            if (combatType == CombatType.RANGE) {
                if (!c.protectingRange()) {
                    damage = 99;
                    accuracyRoll = 100;
                } else if (c.protectingRange()) {
                    damage = 0;
                }
        ...
        if (successfulHit) {
            ...
            if (c.playerEquipment[Player.playerHat] == 22326 && c.playerEquipment[Player.playerChest] == 22327 && c.playerEquipment[Player.playerLegs] == 22328) {
                if (damage > 5) {
                    damage = (int) Math.max(1, (damage * .75));
                }
            }

            if (damage > 10 && c.getCombatItems().elyProc()) {
                damage = (int) Math.max(1, (damage * .75));
            }

            if (damage > 10 && (c.getEquipmentToShow(Player.playerAura) == 10558 || c.hasEquippedSomewhere(29489)) && !c.getPosition().inWild()) {
                damage = (int) Math.max(1, (damage * .85));
            } else if (damage > 10 && (c.playerEquipment[Player.playerHands] == 13372 || c.playerEquipment[Player.playerHands] == 33402) && !c.getPosition().inWild()) {
                damage = (int) Math.max(1, (damage * .85));
            }
        }
        ...
        if (damage == (c.getHealth().getCurrentHealth() - 1)) {
            damage = c.getHealth().getCurrentHealth();
        }

        if (damage > -1) {
            c.appendDamage(npc, damage, damage > 0 ? HitMask.HIT : HitMask.MISS);
    ...
    public static CombatHit applyAutoAttackDamage(NPC npc, NPC c, NPCAutoAttack npcAutoAttack) {
        NPCCombatAttack npcCombatAttack = new NPCCombatAttack(npc, c);
        ...
        int maxHit = (int) (npcAutoAttack.getMaxHit() * damageModifier);
        int damage = maxHit > 0 ? Misc.random(npcAutoAttack.getMinHit(), maxHit) : 0;
    ...
    public static void applyDamage(NPC npc, NPCHandler context) {
        if (npc != null) {
            if (getPlayers().get(npc.oldIndex) == null) {
                return;
            }
            if (npc.isDead()) {
                return;
            ...
            Player c = getPlayers().get(npc.oldIndex);
            ...
            if (c.respawnTimer <= 0) {
                ...
                if (npc.getAttackType() != null) {
                    ...
                    switch (npc.getAttackType()) {
                        ...
                        case MELEE:
                            ...
                            if (c.protectingMelee() && !protectionIgnored) {
                                if (npc.getNpcId() == 5890 || npc.getNpcId() == 10531 || npc.getNpcId() == 10532)
                                    damage /= 3;
                            ...
                            if ((c.hasEquippedSomewhere(10556)
                                    || c.hasEquippedSomewhere(29489)
                                    || c.hasEquippedSomewhere(22954)
                            ...
                            if (c.getItems().isWearingItem(21816) && (IntStream.of(revs).anyMatch(id -> id == npc.getNpcId()))) {
                        ...
                        case DRAGON_FIRE:
                            ...
                            boolean isProtected =
                                    ...
                                    c.getItems().isWearingItem(1540) ||
                                             ...
                                             * Dragon Fire Perk.
                                             */
                                            c.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33118) ||
                                            /**
                        ...
                        case SPECIAL:
                            ...
                            switch (npc.getNpcId()) {
                                ...
                                case 3129:
                                    if (c.protectingMagic() || c.protectingRange() || c.protectingMelee()) {
                                        break;
                                    }
                    ...
                    if (npc.getNpcId() == 6617 || npc.getNpcId() == 6616 || npc.getNpcId() == 6615) {
                        ...
                        if (distanceFromTarget <= 1 && Scorpia.stage > 0 && Server.getNpcs().nonNullStream()
                    ...
                    IntPredicate hasHelmet = id -> c.getItems().isWearingItem(id, Player.playerHat);
                    boolean regularHelm = IntStream.of(c.SLAYER_HELMETS).anyMatch(hasHelmet);
                    boolean imbuedHelm = IntStream.of(c.IMBUED_SLAYER_HELMETS).anyMatch(hasHelmet);

                    if (!regularHelm && !imbuedHelm && !c.getItems().isWearingItem(4168)) {
                        if (npc.getNpcId() == Npcs.DEVIANT_SPECTRE) {
                            int[] toDecrease = { 0, 1, 2, 4, 5, 6 };

                            for (int tD : toDecrease) {
                                c.playerLevel[tD] -= (int) (c.getLevelForXP(c.playerXP[tD]) * .10);
                                if (c.playerLevel[tD] < 0)
                                    c.playerLevel[tD] = 1;
                                c.getPA().refreshSkill(tD);
                                c.getPA().setSkillLevel(tD, c.playerLevel[tD], c.playerXP[tD]);
                    ...
                    if (secondDamage > -1) {
                        c.appendDamage(npc, secondDamage, secondDamage > 0 ? HitMask.HIT : HitMask.MISS);
