...
import static io.kyros.content.bosses.DonorBoss3.burnGFX;
import static io.kyros.model.entity.npc.NPCHandler.*;
...
public class LoadSpell {
    ...
    public static void loadSpell(Player player, NPC npc) {
        ...
        switch (npc.getNpcId()) {
            case 11775:
                if (Boundary.isIn(npc, Boundary.BABA_ZONE) && npc.getNpcAutoAttacks().isEmpty()) {
                    PhantomBaBa.setupAutoAttack(npc);
                }
                break;

            case 10936:
                if (Boundary.isIn(npc, YamaInstance.YAMA_ZONE)) {
                    YamaNPC.setAttacks(npc);
                }
                ...
                break;
            case 12617:
                if (Boundary.isIn(npc, SharathteerkInstance.SHARATH_ZONE)) {
                    SharathteerkNPC.setAttacks(npc);
                }
                break;
            case 11756:
                if (Boundary.isIn(npc, TumekensInstance.TUMEKENS_ZONE)) {
                    TumekensNPC .setAttacks(npc);
                }
                break;
            case 13003:
                Eagle.handleAutoAttacks(npc);
                break;
            case 7221:
                HotDrops.handleAutoAttacks(npc);
                break;
            case 7931:
            case 7932:
            case 7933:
            ...
            case 8172:
                chance = Misc.random(100);
                int distanceToAraphel = player.distanceToPoint(npc.absX, npc.absY);
                if (distanceToAraphel < 3) {
                    if (chance < 25) {
                        npc.setAttackType(CombatType.MAGE);
                        npc.attackTimer = 7;
                        npc.hitDelayTimer = 4;
            ...
            case 3127:
                int r3 = 0;
                if (handler().goodDistance(npc.absX, npc.absY, Server.getPlayers().get(npc.spawnedBy).absX, Server.getPlayers().get(npc.spawnedBy).absY, 1)) {
                    r3 = Misc.random(2);
                } else {
                    r3 = Misc.random(1);
                }
            ...
            case Hespori.NPC_ID:
                ...
                switch (voteboss) {
                    ...
                    case 3:
                    case 4:
                        npc.startAnimation(6513, AnimationPriority.HIGH);
                        npc.setAttackType(CombatType.MELEE);
                        npc.endGfx = -1;
                        npc.projectileId = -1;
                        npc.maxHit = 5;
                    ...
                    case 6:
                    case 7:
                        npc.startAnimation(6514, AnimationPriority.HIGH);
                        npc.setAttackType(CombatType.RANGE);
                        npc.projectileId = -1;
                        npc.endGfx = -1;
                        npc.hitDelayTimer = 4;
                ...
                break;
            case 12812:
                IsleOfTheDamned.minotaurAttack(npc, player);
                break;
            case 12607:
                IsleOfTheDamned.dragonAttack(npc, player);
                break;
            case 853:
                IsleOfTheDamned.icelordAttack(npc, player);
                break;
            case 4923:
                Groot.attack(npc);
                break;
