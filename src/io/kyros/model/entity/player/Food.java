...
public class Food {
     ...
     */
    private static final int FOOD_DELAY = 2;
    ...
    public enum FoodToEat {
        ...
        BANDAGES(4049, 10, "Bandages", 0, "Food", false),
        ANGLERFISH(13441, -1, "Anglerfish", 0, "Food", true),

        BURNT_MANTA(393, 22, "Manta Ray", 0, "Food", false),
        BURNT_SEA_TURTLE(399, 21, "Sea Turtle", 0, "Food", false),
        BURNT_SHARK(387, 20, "Shark", 0, "Food", false),
        BURNT_LOBSTER(381, 12, "Lobster", 0, "Food", false),
        BURNT_KARAMBWAN(3148, 18, "Karambwan", 0, "Food", false),
        BURNT_ANGLERFISH(13443, -1, "Anglerfish", 0, "Food", true),
    ...
    public void eat(int id, int slot) {
        ...
        if(Server.definitionRepository.get(ItemDefinition.class, id).name.startsWith("Burnt")) {
            if(!player.getCurrentPet().hasPerk("uncommon_iron_gut")) {
                player.sendMessage("You need to Iron Stomach pet perk to eat burnt food.");
                return;
            }
        }
        TickTimer consumableTimer = FoodToEat.isComboFood(foodToEat) ? player.getComboTimer() : player.getFoodTimer();
        int attackDelay = FoodToEat.isComboFood(foodToEat) ? 1 : 2;

        consumableTimer.reset();
        if (!player.getCombatPrayer().isPrayerActive(Prayer.METABOLISE.getId()))
            player.attackTimer += attackDelay;
        player.attacking.reset();
        player.startAnimation(829);
        player.getItems().deleteItem(id, slot, 1);
        player.getPA().sendSound(317);
        ...
        if (id != 13441) {
            ...
            if(Server.definitionRepository.get(ItemDefinition.class, id).name.startsWith("Burnt")) {
                if(player.getCurrentPet().findPetPerk("uncommon_iron_gut").isHit()) {
                    player.sendMessage("Your burnt food heals you for double the amount!.");
                    int heal = foodToEat.getHeal();
                    if (player.getCurrentPet().findPetPerk("common_poison_food").getValue() > 0) {
                        heal = (int) (heal * 0.8);//20% less food health.
                    }
