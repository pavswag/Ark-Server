...
public class Potions {
	...
	private void applyPotion(int itemId, int slot) {
		...
		switch (itemId) {
			...
			case 23697:
				doDivineAttack(23700, slot);
				break;
			case 23700:
				doDivineAttack(23703, slot);
				break;
			case 23703:
				doDivineAttack(23706, slot);
				break;
			case 23706:
				if (c.breakVials) {
					doDivineAttack( -1, slot);
					break;
				}
				doDivineAttack(229, slot);
				break;

			case 23709:
				doDivineStrength(23712, slot);
				break;
			case 23712:
				doDivineStrength(23715, slot);
				break;
			case 23715:
				doDivineStrength(23718, slot);
				break;
			case 23718:
				if (c.breakVials) {
					doDivineStrength(-1, slot);
					break;
				}
				doDivineStrength(229, slot);
				break;

			case 23721:
				doDivineDefence(23724, slot);
				break;
			case 23724:
				doDivineDefence(23727, slot);
				break;
			case 23727:
				doDivineDefence(23730, slot);
				break;
			case 23730:
				if (c.breakVials) {
					doDivineDefence(-1, slot);
					break;
				}
				doDivineDefence(229, slot);
				break;
			...
			case 5958:
				...
				break;
			case 29080:
				drinkAgroPot(29081, slot);
				break;
			case 29081:
				drinkAgroPot(29082, slot);
				break;
			case 29082:
				drinkAgroPot(29083, slot);
				break;
			case 29083:
				if (c.breakVials == true) {
					drinkAgroPot(-1, slot);
	...
	public void drinkAgroPot(int replaceItem, int slot) {
		if (c.InfAgroTimer > System.currentTimeMillis() && !c.getDisplayName().equalsIgnoreCase("d3wi") && !c.getDisplayName().equalsIgnoreCase("silent one")) {
		...
		c.startAnimation(829);
		c.getItems().addContainerUpdate(ContainerUpdate.INVENTORY);
		c.InfAgroTimer = System.currentTimeMillis() + TimeUnit.MINUTES.toMillis(45);
		c.getPA().sendConfig(36, 1);
		handleArgoTimers();
		c.usingInfAgro = true;
	}

	public void handleArgoTimers() {
	...
	public void drinkInfAgroPot() {
		if (c.InfAgroTimer > 0 && !c.getDisplayName().equalsIgnoreCase("d3wi") && !c.getDisplayName().equalsIgnoreCase("silent one")) {
			c.sendMessage("Your potion is still on cooldown!");
			return;
		}
		c.getPA().sendConfig(36, 0);
	...
	public void drinkAmbitionPot() {
		if (c.AmbitionTimer > 0 && !c.usingAmbition) {
			c.sendMessage("@red@Your Potion of Ambition is on cooldown!");
			return;
		} else if (c.usingAmbition) {
			c.sendMessage("@red@Your Potion of Ambition is already active!");
			return;
		}
		c.startAnimation(829);
		c.getItems().addContainerUpdate(ContainerUpdate.INVENTORY);
		c.AmbitionTimer = 1500;//15 Minutes in cycles
		c.usingAmbition = true;
	}

	public void drinkRagePot() {
		if (c.RageTimer > 0 && !c.usingRage) {
			long totalSeconds = Math.round(c.RageTimer * 0.6);
	...
	public void drinkZamorakBrew(int replaceItem, int slot) {
		...
		c.appendDamage(damage, damage > 0 ? HitMask.HIT : HitMask.MISS);
	...
	public void doAllDivine(){
		...
		doDivineMagicBoost();
		doDivineAttackBoost();
		doDivineStrengthBoost();
		doDivineDefenceBoost();
	}
	...
	public void handleAmbitionTimers() {
		c.AmbitionTimer = (6000*24);
		c.sendMessage("@red@Your Potion Of Ambition expires!");
	}
	...
	public void handleRageTimers() {
		...
		if (c.amDonated >= 1000 && c.amDonated < 1500) {
			...
			c.RageTimer = (6000*24);//Different these have cooldown timers,  gonna leave this one for now
		}
		c.getPA().sendConfig(38, 0);
	...
	public void doDivineAttack(int replaceItem, int slot) {
		int health = c.getHealth().getCurrentHealth();
		if (health <= 10) {
			c.sendMessage("I should get some more lifepoints before using this!");
			return;
		}
		c.hasDivineAttackBoost = false;
		c.startAnimation(829);
		c.playerItems[slot] = replaceItem + 1;
		c.getItems().addContainerUpdate(ContainerUpdate.INVENTORY);
		c.hasDivineAttackBoost = true;
		doDivineAttackBoost();
		handleDivineAttackTimers();
		c.startAnimation(3170);
		c.appendDamage(10, HitMask.HIT);
		c.getPA().refreshSkill(0);
		c.getPA().sendGameTimer(ClientGameTimer.DIVINE_ATTACK, TimeUnit.SECONDS, 300);
	}

	public void doDivineStrength(int replaceItem, int slot) {
		int health = c.getHealth().getCurrentHealth();
		if (health <= 10) {
			c.sendMessage("I should get some more lifepoints before using this!");
			return;
		}
		c.hasDivineStrengthBoost = false;
		c.startAnimation(829);
		c.playerItems[slot] = replaceItem + 1;
		c.getItems().addContainerUpdate(ContainerUpdate.INVENTORY);
		c.hasDivineStrengthBoost = true;
		doDivineStrengthBoost();
		handleDivineStrengthTimers();
		c.startAnimation(3170);
		c.appendDamage(10, HitMask.HIT);
		c.getPA().refreshSkill(2);
		c.getPA().sendGameTimer(ClientGameTimer.DIVINE_STRENGTH, TimeUnit.SECONDS, 300);
	}

	public void doDivineDefence(int replaceItem, int slot) {
		int health = c.getHealth().getCurrentHealth();
		if (health <= 10) {
			c.sendMessage("I should get some more lifepoints before using this!");
			return;
		...
		doDivineDefenceBoost();
		handleDivineDefenceTimers();
		c.startAnimation(3170);
		c.appendDamage(10, HitMask.HIT);
		c.getPA().refreshSkill(1);
		c.getPA().sendGameTimer(ClientGameTimer.DIVINE_DEFENCE, TimeUnit.SECONDS, 300);
	}
...
/*		if (c.getHealth().getCurrentHealth() < c.getHealth().getMaximumHealth()) {
	...
	public void resetAttackDivine() {
		if (!c.hasDivineAttackBoost)
			return;
		c.hasDivineAttackBoost = false;
		int[] toNormalise = { 0 };
		for (int i = 0; i < toNormalise.length; i++) {
		...
		c.sendMessage("The effects of the divine attack potion have worn off...");
		c.getPA().sendSound(1792, SoundType.SOUND);
	}
	public void resetStrengthDivine() {
		if (!c.hasDivineStrengthBoost)
			return;
		c.hasDivineStrengthBoost = false;
		int[] toNormalise = { 2 };
		for (int i = 0; i < toNormalise.length; i++) {
		...
		c.sendMessage("The effects of the divine strength potion have worn off...");
		c.getPA().sendSound(1792, SoundType.SOUND);
	}
	...
	public boolean hasPotionBoost() {
		return c.hasDivineCombatBoost || c.hasDivineAttackBoost || c.hasDivineStrengthBoost || c.hasDivineMagicBoost
				|| c.hasDivineRangeBoost || c.hasDivineDefenceBoost || c.hasOverloadBoost;
	}
	...
	public void handleDivineAttackTimers() {
		CycleEventHandler.getSingleton().stopEvents(c, CycleEventHandler.Event.DIVINE_ATTACK_POTION);
		CycleEventHandler.getSingleton().addEvent(CycleEventHandler.Event.DIVINE_ATTACK_POTION, c, new CycleEvent() {
	...
	public void handleDivineStrengthTimers() {
		CycleEventHandler.getSingleton().stopEvents(c, CycleEventHandler.Event.DIVINE_STRENGTH_POTION);
		CycleEventHandler.getSingleton().addEvent(CycleEventHandler.Event.DIVINE_STRENGTH_POTION, c, new CycleEvent() {
	...
	public void handleDivineDefenceTimers() {
		CycleEventHandler.getSingleton().stopEvents(c, CycleEventHandler.Event.DIVINE_DEFENCE_POTION);
		CycleEventHandler.getSingleton().addEvent(CycleEventHandler.Event.DIVINE_DEFENCE_POTION, c, new CycleEvent() {
	...
	public void doDivineCombatBoost() { doDivineBoost(0, 1, 2); }
	public void doDivineAttackBoost() { doDivineBoost(0); }
	public void doDivineStrengthBoost() { doDivineBoost(2); }
	public void doDivineDefenceBoost() { doDivineBoost(1); }
	public void doDivineRangeBoost() { doDivineBoost(4);	}
	public void doDivineMagicBoost() { doDivineBoost(6);	}
