...
public class OperateItem implements PacketType {
	...
	public void processPacket(Player c, int packetType, int packetSize) {
		...
		if (Halloween.handleBucket(c, itemId)) {
			return;
		}
...
//                TeleportInterface.sendMonsterTab(player);
			...
			case 11284:
				...
				if (dfsEffect.isExecutable(c, null)) {
					...
					if (c.playerAttackingIndex > 0) {
                        Player target = getPlayers().get(c.playerAttackingIndex);
				...
				case 26914:
					c.collectNecklace = !c.collectNecklace;
					c.sendMessage("[Collector Necklace] all drops will now be " + (c.collectNecklace ? "sent to your bank." : "shared between your bank and inventory."));
					break;
			...
			case 9763:
				...
				case 19707:
					...
					if (c.wildLevel > 30 && c.getCurrentPet().getNpcId() != 2316) {
				...
				case 2566:
					if (c.getMode().equals(Mode.forType(ModeType.WILDYMAN)) || c.getMode().equals(Mode.forType(ModeType.HARDCORE_WILDYMAN))) {
						c.getPA().spellTeleport(3135, 3628, 0, false);
						break;
					}
					switch (slot) {
						case 1:
