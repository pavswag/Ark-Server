package io.kyros.model.entity.player.packets.dialogueoptions;
...
import io.kyros.content.minigames.barrows.RoomLocation;
...
import io.kyros.model.entity.player.PrayerScrolls;
...
public class TwoOptions {
	...
	public static void handleOption1(Player c) {  //see This handles option 1 if u scroll down ull see the section two
		...
		case 141315:
					ManticoreInstance inst = new ManticoreInstance(c, ManticoreInstance.boundary);
					inst.enter(c, inst);
					c.sendMessage("You have teleported to Manticore");
					c.getPA().closeAllWindows();
			break;
...
//					3);
			...
			case 12791:
				...
				case LootingBag.LOOTING_BAG_OPEN:
					if (!c.getPosition().inWild()) {
						c.getPA().closeAllWindows();
						c.sendMessage("@dre@You can't do this while outside the wilderness.");
						return;
					}
		...
		case 784:
			...
			break;
			case 956://1 scroll
				if (c.getPosition().inWild() || c.getPosition().inDuelArena() || Server.getMultiplayerSessionListener().inAnySession(c) || c.tradeBanned) {
					return;
				}
				if (other.getPosition().inWild() || other.getPosition().inDuelArena() || Server.getMultiplayerSessionListener().inAnySession(other) || other.tradeBanned) {
					return;
				}
				...
				if (c.getItems().playerHasItem(956, 1)) {
					Discord.writeServerSyncMessage("[USE ON] "+ c.getDisplayName() +" gave " + other.getDisplayName() + " a $1 scroll.");
					c.getItems().deleteItem(956, 1);
					other.getItems().addItem(956, 1);
					c.gfx100(263);
					other.gfx100(263);
					other.sendMessage("You were given a $1 donator scroll.");
					c.sendMessage("Your $1 donator scroll has been given away.");
					c.getPA().closeAllWindows();
				}
				break;
		case 4005://5 scroll
			...
			if (c.getItems().playerHasItem(6769, 1)) {
				Discord.writeServerSyncMessage("[USE ON] "+ c.getDisplayName() +" gave " + other.getDisplayName() + " a $5 scroll.");
		...
		case 4006://10 scroll
			...
			if (c.getItems().playerHasItem(2403, 1)) {
				Discord.writeServerSyncMessage("[USE ON] "+ c.getDisplayName() +" gave " + other.getDisplayName() + " a $10 scroll.");
		...
		case 4007://25 scroll
			...
			if (c.getItems().playerHasItem(2396, 1)) {
				Discord.writeServerSyncMessage("[USE ON] "+ c.getDisplayName() +" gave " + other.getDisplayName() + " a $25 scroll.");
		...
		case 4008://50 scroll
			...
			if (c.getItems().playerHasItem(786, 1)) {
				Discord.writeServerSyncMessage("[USE ON] "+ c.getDisplayName() +" gave " + other.getDisplayName() + " a $50 scroll.");
		...
		case 4009://100 scroll
			...
			if (c.getItems().playerHasItem(761, 1)) {
				Discord.writeServerSyncMessage("[USE ON] "+ c.getDisplayName() +" gave " + other.getDisplayName()  + " a $100 scroll.");
		...
		case 4010://250 scroll
			...
			if (c.getItems().playerHasItem(607, 1)) {
				Discord.writeServerSyncMessage("[USE ON] "+ c.getDisplayName() +" gave " + other.getDisplayName() + " a $250 scroll.");
		...
		case 4011://500 scroll
			...
			if (c.getItems().playerHasItem(608, 1)) {
				Discord.writeServerSyncMessage("[USE ON] "+ c.getDisplayName() +" gave " + other.getDisplayName() + " a $500 scroll.");
		...
		if (c.dialogueAction == 124) {
			if (Boundary.isIn(c, Boundary.EDGEVILLE_PERIMETER) || Boundary.isIn(c, Boundary.DONATOR_ZONE) || Boundary.isIn(c, Boundary.DONATOR_ZONE_NEW) || Boundary.isIn(c, Boundary.ONYX_ZONE) || Boundary.isIn(c, Boundary.LEGENDARY_ZONE)) {
	...
	public static void handleOption2(Player c) { //under this code is the dialogues OpTion two codIng ooh okay, and its the same id
		...
		case 141315:
			JavelinColossus jav = new JavelinColossus(c, JavelinColossus.Area);
			jav.enter(c, jav);
			c.sendMessage("You have teleported to Javelin Colossus");
			c.getPA().closeAllWindows();
			break;
