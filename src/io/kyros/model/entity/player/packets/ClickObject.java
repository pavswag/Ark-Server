...
import io.kyros.content.item.lootable.impl.SerenChest;
import io.kyros.content.item.lootable.impl.UnbearableChest;
...
import io.kyros.content.skills.agility.impl.rooftop.RooftopPollnivneach;
import io.kyros.content.skills.hunter.impling.PuroPuro;
import io.kyros.content.skills.runecrafting.ouriana.OurianaAltar;
import io.kyros.content.skills.runecrafting.ouriana.OurianaBanker;
import io.kyros.content.skills.slayer.LarrensKey;
...
import io.kyros.model.tickable.impl.WalkToTickable;
import io.kyros.model.world.objects.GlobalObject;
import io.kyros.util.logging.player.ClickObjectLog;
...
public class ClickObject implements PacketType {
    ...
    private static void walkTo(Player player, int option) {
        ...
        if (object != null) {
            ...
            } else if (object.getId() == 11663) {
//                PathFinder.getPathFinder().findRoute(player, object.getX(), object.getY(), true, 1, 1);
                player.setTickable((container, plr) -> {
                    if (plr.distance(object.getPosition()) < 15) {
                        finishObjectClick(plr, option, object);
                        container.stop();
                    }
                });
            ...
            } else if (Arrays.stream(RooftopPollnivneach.POLLNIVNEACH_OBJECTS).anyMatch(i -> i == player.objectId)) {
                PathFinder.getPathFinder().findRoute(player, object.getX(), object.getY(), true, 1, 1);
                player.setTickable(((container, player1) -> {
                    if (player1.distance(object.getPosition()) < 2.5) {
                        finishObjectClick(player1, option,object);
                        container.stop();
                    }
                }));
            } else if (player.objectId == 54161 || player.objectId == 54274) {
                player.objectDistance = 6;
//                PathFinder.getPathFinder().findRoute(player, object.getX(), object.getY(), true, 1, 1);
                player.setTickable(((container, player1) -> {
                    if (player1.distance(object.getPosition()) < 5) {
                        finishObjectClick(player1, option,object);
                        container.stop();
                    }
                }));
...
//            System.out.println("Movement processed");
        } else if (player.objectId > 0 && player.objectX > 0 && player.objectY > 0) {
            WorldObject obj = new WorldObject(player.objectId, player.objectX, player.objectY, player.getHeight(), player.clickObjectType, 0);
            Position size = obj.getObjectSize();
            player.setTickable(new WalkToTickable(player, obj.getPosition(), size.getX(), size.getY(), player1 -> finishObjectClick(player1, option, obj)));
        } else {
            player.stopMovement();
        }
    }
    ...
    public void processPacket(final Player c, int packetType, int packetSize) {
        ...
        if (c.isFping()) {
             ...
             */
            System.out.println("inside of flower poker");
            return;
        }
        ...
        if (c.teleTimer > 0) {
            System.out.println("TeleTimer is greater than 0");
            return;
        }
        c.xInterfaceId = -1;
...
//                System.out.println(c.clickObjectType +" / "+ c.objectX +" / "+ c.objectId +" / "+ c.objectY);
                c.objectDistance = 1;
                walkTo(c, 1);
                break;
            case SECOND_CLICK:
    ...
    private static void finishObjectClick(Player c, int option, WorldObject worldObject) {
        ...
        if (c.getShadowcrusadeContainer().handleClickObject(worldObject)) {
            return;
        }
        ...
        switch (option) {
            case 1:
                ...
                if (io.kyros.content.dwarfleaguecannon.Cannon.clickObject(c, c.objectId, new Position(c.objectX, c.objectY, c.getHeight()), 1)) {
                    return;
                }
				...
				switch (c.objectId) {
                    ...
                    case 30943: {
                        c.getUpgradeInterface().openInterface(UpgradeMaterials.UpgradeType.WEAPON);
                        return;
                    }

                    case 34648:
                        c.getItems().addItemUnderAnyCircumstance(2162, 1);
                        Server.getGlobalObjects().add(new GlobalObject(-1, c.objectX, c.objectY, 0, 0, 10, 14, 34648));
                        return;

                    case 30944: {
                        c.getFusionSystem().openInterface(FusionTypes.WEAPON);
                        return;
                    }
                    ...
                    case 33223:
                        ...
                        if (Hespori.TOTAL_ESSENCE_BURNED < Hespori.ESSENCE_REQUIRED) {
                            c.sendMessage("Hespori requires " + (Hespori.ESSENCE_REQUIRED - Hespori.TOTAL_ESSENCE_BURNED) + " " +
                    ...
                    case 33730:
                        ...
                        if (c.getItems().playerHasItem(Hespori.KEY)) {
                            c.getPA().spellTeleport(3072 + randomTele, 3499 + randomTele, 0, false);
                            return;
                        }
                        break;
...
//						break;
                    ...
                    case 53222:
                        ...
                        if (c.getItems().playerHasItem(PirateChest.KEY, 1)) {
                            c.objectDistance = 3;
                            new PirateChest().roll(c);
                            return;
                        }
                        c.getDH().sendStatement("@red@You need a Pirate key to open this.");
                        break;
                    ...
                    case 26740:
                    case 47347:
                        if (c.objectId == 26739 && c.objectX == 3430 && c.objectY == 4824) {
                            return;
                        }
...
//                        c.getPA().movePlayer(new Coordinate(3080, 3510));
                    ...
                    case 25018:
                    case 25029:
                        PuroPuro.magicalWheat(c, worldObject);
                        break;
                    case 21728:
...
/*                    case 29735:// Basic training
                        if (c.objectX != 3306 && c.objectY != 5174) {
                    ...
                    case 23566:// rune and addy dragons
                        ...
                        break;
                    case 39643:  //Ferox stairs.
                        c.getPA().movePlayer(3146, 3636, 1);
                        break;
                    case 39650:  //Ferox stairs.
                        c.getPA().movePlayer(3143, 3636, 0);
                        break;
            ...
            case 2:
                ...
                if (io.kyros.content.dwarfleaguecannon.Cannon.clickObject(c, c.objectId, new Position(c.objectX, c.objectY, c.getHeight()), 2)) {
                    return;
                }
                if (OurianaBanker.clickObject(c, c.objectId, option)) {
                    return;
            ...
            case 3:
                ...
                if (io.kyros.content.dwarfleaguecannon.Cannon.clickObject(c, c.objectId, new Position(c.objectX, c.objectY, c.getHeight()), 3)) {
                    return;
                }
