...
import io.kyros.content.displayname.ChangeDisplayName;
...
import io.kyros.content.skills.slayer.DuoMode;
...
public class ItemOptionTwo implements PacketType {
	...
	public void processPacket(Player player, int packetType, int packetSize) {
		...
		if (player.getPetManagement().isCashMoney(itemId)) {
			player.getPetManagement().removePets();
			return;
		}
		...
		if (ChangeDisplayName.clickChangeNameItem(player, itemId))
			return;
		Server.pluginManager.triggerEvent(new io.kyros.script.event.impl.ItemAction(player, itemId, 2));
		...
		case 7509:
            ...
            if (player.getHealth().getCurrentHealth() <= 10 && player.getHealth().getCurrentHealth() > 1 ) {
                player.appendDamage(1, HitMask.HIT);
			...
			player.getPA().sendSound(169);
            int currentHealth = player.getHealth().getCurrentHealth() / 10;
            player.appendDamage(currentHealth, HitMask.HIT);
            break;
		case 9762:
        ...
        case 10557:
        	...
        	break;
        case 10558:
        	player.sendMessage("@red@Defender Icon will reduce all incoming damage by 15%.");
        	break;
        case 10559:
        	player.sendMessage("@red@Healer Icon shares the same effects as guthans.");
        	break;
		case 10832:
		...
		case 10835:
			...
			break;
		case 22993:
			player.getSeedBox().openall(player);
			break;
			case 11738:
				player.getHerbBox().openall(player);
				break;
		case 9780:
		...
		case Items.CRAWS_BOW:
			...
			case 33161:
			case 33162:
				ChristmasWeapons.handleItemOption(player, itemId, 2);
		case 6199:
		case 6828:
		case 13346:
		...
		case 21347:
			...
			case 13660:
				if(player.wildLevel > Configuration.NO_TELEPORT_WILD_LEVEL && player.getCurrentPet().getNpcId() != 2316) {
