...
public class NpcOptionTwo {
	...
	public static void handleOption(Player player, int npcType) {
		...
		Server.pluginManager.triggerEvent(new io.kyros.script.event.impl.NpcOptionTwo(npc, player));
		switch (npcType) {
		...
		case DailyRewardsDialogue.DAILY_REWARDS_NPC:
			...
			break;
			case 10529:
				player.sendMessage("@bla@You currently have @red@"+player.foundryPoints+"@bla@ Nomad Points.");
		...
		case 1306:
			...
			break;
			case 2306:
				player.getPA().startTeleport(1951, 6263, 0, "foundry", false);
				break;
			case 7677:
		...
		case 2989:
//			player.sendMessage("Disabled right now sorry.");
	...
	/*	case 2897:
		...
		case 6970:
			...
			player.getThieving().doStealNpc(Thieving.Pickpocket.MAN, npc);
			break;
		case 3295: //for diary

			if (Boundary.isIn(player, Boundary.ARDOUGNE)) {
				player.getThieving().doStealNpc(Thieving.Pickpocket.HERO, npc);
		...
		case 6987:
			...
			break;
		case 3550:
			player.getThieving().doStealNpc(Thieving.Pickpocket.MENAPHITE_THUG, npc);
			break;
		case 6094:
			player.getThieving().doStealNpc(Thieving.Pickpocket.GNOME, npc);
			break;
		case 3106:
			player.getThieving().doStealNpc(Thieving.Pickpocket.HERO, npc);
			break;
