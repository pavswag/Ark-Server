...
public class ItemOnPlayer implements PacketType {
	...
	public void processPacket(Player c, int packetType, int packetSize) {
		...
		if (c.teleTimer > 0)
			return;
		if (playerIndex > getPlayers().size()) {
			return;
		}
		if (slotId > c.playerItems.length) {
			return;
		}
		if (getPlayers().get(playerIndex) == null) {
			return;
		}
        ...
        Player other = getPlayers().get(playerIndex);
		if (other == null) {
			return;
		}
		...
		if (c.getRights().isOrInherits(Right.HELPER) && itemId != 5733 && itemId != 6713) {
