...
public class ContainerAction4 implements PacketType {
	...
	public void processPacket(Player c, int packetType, int packetSize) {
		...
		if(interfaceId == 12463) {
			c.getPA().sendEnterAmount("Enter amount to add", (plr, amt) -> {
				EventManager.add(c, removeId, amt);
			});
		}
		//Server.pluginManager.triggerEvent(new ItemContainerOption(c, interfaceId, removeSlot, removeId, 4));
		if (c.getInterfaceEvent().isActive()) {
			c.sendMessage("Please finish what you're doing.");
			return;
		}
		if (interfaceId == 48500) {
			c.getTradePost().handleInput(interfaceId, 4, removeId);
			return;
		}
		if (interfaceId == 26022) {
			c.getTradePost().handleInput(interfaceId, 4, removeSlot);
			return;
		}
