...
import io.kyros.content.Censor;
import io.kyros.content.bosses.grotesqueguardians.GrotesqueInstance;
import io.kyros.content.bosses.hespori.*;
...
import io.kyros.content.event.eventcalendar.EventCalendarHelper;
...
import io.kyros.content.worldevent.impl.WildernessBossWorldEvent;
...
import io.kyros.script.event.impl.CommandEvent;
import io.kyros.sql.eventcalendar.queries.AddWinnerQuery;
import io.kyros.util.Misc;
import io.kyros.util.logging.player.ClanChatLog;
import io.kyros.util.logging.player.CommandLog;
...
public class Commands implements PacketType {
    ...
    public void processPacket(Player c, int packetType, int packetSize) {
        ...
        try {
            ...
            if (Server.getMultiplayerSessionListener().inAnySession(c) && !c.getRights().isOrInherits(Right.STAFF_MANAGER)) {
            ...
            boolean isManagment = c.getRights().isOrInherits(Right.ADMINISTRATOR, Right.COMMUNITY_MANAGER, Right.STAFF_MANAGER, Right.GAME_DEVELOPER) || Server.isDebug();

            boolean isTeam = c.getRights().isOrInherits(Right.ADMINISTRATOR, Right.COMMUNITY_MANAGER, Right.STAFF_MANAGER, Right.GAME_DEVELOPER, Right.MODERATOR) || Server.isDebug();
            ...
            if (isManagment) {
                ...
                switch (args[0]) {
                    ...
                    case "killhespori":
                        ...
                        Hespori.isWeak = true;
                        NPCHandler.getNpc(Npcs.HESPORI).appendDamage(null, 500_000, HitMask.HIT_MAX);
                        return;
            ...
            if (isTeam) {
                ...
                if (playerCommand.startsWith("endachieve")) {
                    c.sendMessage("Ended achieve event.");
                    new AchieveBonus().deactivate();
                }
...
/*            if (playerCommand.startsWith("filter")) {
            ...
            if (playerCommand.equals("killme")) {
                ...
                c.appendDamage(c.getHealth().getCurrentHealth(), HitMask.HIT_MAX);
            }

            if (playerCommand.equals("pots") && c.getRights().contains(Right.STAFF_MANAGER)) {
                c.getItems().addItem(2445, 10000);
                c.getItems().addItem(12696, 10000);
            }
            if (playerCommand.equals("food") && c.getRights().contains(Right.STAFF_MANAGER)) {
                c.getItems().addItem(386, 10000);
            }

            if (playerCommand.startsWith("maxmelee") && c.getRights().contains(Right.STAFF_MANAGER)) {
            ...
            if (playerCommand.startsWith("maxrange") && c.getRights().contains(Right.STAFF_MANAGER)) {
                for (int slot = 0; slot < c.playerItems.length; slot++)
                    if (c.playerItems[slot] > 0 && c.playerItemsN[slot] > 0) {
                        c.getItems().addToBank(c.playerItems[slot] - 1, c.playerItemsN[slot], false);
            ...
            if (playerCommand.toLowerCase().contentEquals("sandesus")) {
                c.getPA().sendFrame126("https://www.youtube.com/@SandesuSRS", 12000);
            }
            ...
            if (playerCommand.startsWith("update") && c.getRights().isOrInherits(Right.GAME_DEVELOPER)) {
                if (!isManagment) {
                    c.sendMessage(NO_ACCESS);
                    return;
                }
            ...
            if (playerCommand.equals("rights")) {
                c.sendMessage("isOwner: " + c.getRights().contains(Right.STAFF_MANAGER));
                ...
                c.sendMessage("isExreme_Donor: " + c.getRights().contains(Right.Super_Donator));
            ...
            if (playerCommand.startsWith("giverights")) {
                ...
                try {
                    ...
                    boolean found = false;
                    Player p = Server.getPlayers().search(plr -> plr.getDisplayName().equalsIgnoreCase(target)).orElse(null);
                    if(p != null) {
            ...
            if (playerCommand.startsWith("foe") && c.getRights().isOrInherits(Right.STAFF_MANAGER)) {
                if (Boundary.isIn(c, Boundary.TzHaar_Foundry)) {
                    c.sendMessage("@bla@[@red@Nomad@bla@]@blu@ Remember, any exchanges are @red@final@blu@, items will not be returned.");
                    c.sendMessage("@bla@[@red@Nomad@bla@] @blu@Click an item in your inventory to offer. Use the green arrow to confirm.");
                    ...
                    if (c.getItems().playerHasItem(771)) {
                        c.sendMessage("@bla@[@red@Nomad@bla@]@blu@The @cya@ancient branch@blu@ must be used with only 1 item at a time.");
                    }
                } else {
...
/*            if (playerCommand.startsWith("sound")) {
            ...
            if (playerCommand.startsWith("multihittest") && c.getRights().isOrInherits(Right.STAFF_MANAGER)) {
...
*/
            ...
            if (playerCommand.startsWith("nightmaregear") && c.getRights().isOrInherits(Right.STAFF_MANAGER)) {
                if (!isManagment) {
                    c.sendMessage(NO_ACCESS);
                    return;
                }
            ...
            if (playerCommand.startsWith("starter") && c.getRights().isOrInherits(Right.STAFF_MANAGER)) {
                if (!isManagment && !Server.getConfiguration().getServerState().isOpenSpawning()) {
                    c.sendMessage(NO_ACCESS);
                    return;
                }
            ...
            if (playerCommand.startsWith("stack") && c.getRights().isOrInherits(Right.STAFF_MANAGER)) {
                if (!isManagment && !Server.getConfiguration().getServerState().isOpenSpawning()) {
                    c.sendMessage(NO_ACCESS);
                    return;
                }
            ...
            if (playerCommand.startsWith("tut")) {
                ...
                c.start(new TutorialDialogue(c, true, true));
            }
            ...
            if (playerCommand.startsWith("killhespori")) {
                ...
                HesporiSpawner.getHespori().appendDamage(Integer.MAX_VALUE, HitMask.HIT_MAX);
            }
            ...
            Server.pluginManager.triggerEvent(new CommandEvent(c, args[0], newArgs));
        } catch (Exception e) {
            e.printStackTrace();
