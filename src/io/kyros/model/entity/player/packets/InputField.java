...
public class InputField implements PacketType {
	...
	public void processPacket(Player player, int packetType, int packetSize) {
		...
		if (player.debugMessage) {
			player.sendErrorMessage("interfaceId = " + id + ", text = " + text);
		}
		if (SmsManager.isInputField(player, id, text)) {
			return;
		}
		if(player.staffPanel.handleTextInput(id, text))
			return;
		...
		case 59527:
			...
			List<Player> staff = Server.getPlayers().nonNullStream().filter(Objects::nonNull).filter(p -> p.getRights().isOrInherits(Right.HELPER)).collect(Collectors.toList());
