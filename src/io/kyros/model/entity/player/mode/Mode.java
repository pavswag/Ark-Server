...
public abstract class Mode {
	...
	public boolean isHardcoreIronman() {
		return type == ModeType.HC_IRON_MAN || type == ModeType.ROGUE_HARDCORE_IRONMAN/* || type == ModeType.HARDCORE_WILDYMAN*/;
	}
	...
	private static final List<Mode> MODES = ImmutableList
			.copyOf(Arrays.asList(new RegularMode(ModeType.STANDARD),
					...
					new OsrsIronman(ModeType.OSRS_IRONMAN),
					new RogueMode(ModeType.ROGUE),
					new Wildyman(ModeType.WILDYMAN),
					new HardcoreWildyman(ModeType.HARDCORE_WILDYMAN),
	...
	/**
	 * Determines if the {@link #type} is equal to {@link ModeType}
	 *
	 * @return {@code true} if the player is of this type, otherwise {@code false}
	 */
	...
	public boolean isOsrs() {
		return type == ModeType.OSRS || type == ModeType.OSRS_IRONMAN;
	}

	public boolean is5x() {
		return type == ModeType.ROGUE || type == ModeType.ROGUE_HARDCORE_IRONMAN || type == ModeType.HARDCORE_WILDYMAN || type == ModeType.ROGUE_IRONMAN || type == ModeType.GROUP_IRONMAN;
	}
