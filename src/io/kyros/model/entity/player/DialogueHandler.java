...
import io.kyros.content.skills.slayer.*;
...
public class DialogueHandler {
	...
	public void sendDialogues(int dialogue, int npcId) {
		...
		case 3299:
			...
			break;
		case 33900:
			sendOption3("I would like a wilderness task.","I would like to extend my current task. (15m)","Nevermind");
		...
		case 5998:
			sendNpcChat3("Only @cr4@@red@Donators @bla@can access the Donator Zone.",
					"Interested in becoming an  @cr4@@red@Donator@bla@?",
					"Go to the bottom of your quest tab and click the @cr19@@yel@Store.", c.talkingNpc,
					"@cr4@Donation Informative@cr4@");
			c.nextChat = 0;
			break;

		case 5999:
			sendNpcChat3("Welcome to the @cr4@@red@Donator Zone.", "If you want to get inside,",
					"All you need to do is type ::dz .", c.talkingNpc, "@cr4@Donation Informative@cr4@");
			c.nextChat = 0;
			break;
		...
		case 55874:
			...
			break;
		case 909:
			sendOption2("Manticore", "Javelin Colossus");
			c.dialogueAction = 141315;
			c.nextChat = 0;
			break;
		...
		case 14410:
			...
			break;
		case 14411:
			sendOption5("Seers Rooftop Course (Lvl 60)", "Pollnivneach Rooftop Course (Lvl 70)",  "Ardougne Rooftop Course (Lvl 90)", "", "Back");
		...
		case 9999:
			...
			break;
		case 11000:
			Player o = Server.getPlayers().get(c.otherDiceId);
		...
		// break;
		case 11003:
			o = io.kyros.Server.getPlayers().get(c.otherDiceId);
		...
		case 551:
			sendNpcChat3(
					...
					"There are currently " + Boundary.getPlayersInBoundary(new Boundary(2980, 10048, 3473, 10373))
		...
		case 643:
			...
			break;
		case 645:
			sendNpcChat("You've made it through!", "You have the option to play as an <col=" + Right.IRONMAN + ">@cr12@</img>Iron Man</col>, <col=" + Right.GROUP_IRONMAN + ">@cr27@</img>GIM</col>",
                    "<col=" + Right.ULTIMATE_IRONMAN + ">@cr13@</img>Ultimate Iron Man</col>, <col=" + Right.HC_IRONMAN + ">@cr9@</img>Hardcore Iron Man</col>, or neither.", "Choose from the following interface.");
            c.nextChat = 646;
			break;
		...
		case 7286:
			...
			case 7287:
				c.getDH().sendOption5("@red@Which boss would you like to instance?","VoteBoss (5 points)", "DonorBoss (25 points)", "Durial (5 points)", "Groot (15 points)", "@blu@Next Page");
				c.dialogueAction = 7287;
				break;
			case 7288:
				c.getDH().sendOption5("@red@Which boss would you like to instance?", "BaBa (30 points)", "Manticore (10 points)", "Javelin Colossus (10 points)", "Nex (5 points)", "@blu@First Page");
				c.dialogueAction = 7288;
				break;
		...
		case 121313: // city
			...
			case 121314:
				sendNpcChat("Welcome to the TzHaar Foundry","Here we take your item's and turn them into points!");
				c.start(new DialogueBuilder(c)
						.statement("Welcome to the TzHaar Foundry","Here we take your item's and turn them into points!")
						.option(
								new DialogueOption("Access Foundry.", plr -> {
									if (!plr.getRecentlyDissolvedItems().isEmpty() && !plr.getRecentlyDissolvedPrices().isEmpty()) {
										if (plr.getRecentlyDissolvedItems().size() > 6 && plr.getRecentlyDissolvedPrices().size() > 6) {
											plr.getRecentlyDissolvedItems().remove(0);
											plr.getRecentlyDissolvedPrices().remove(0);
										}
										for (int i = 0; i < plr.getRecentlyDissolvedItems().size(); i++) {
											plr.getPA().itemOnInterface(plr.getRecentlyDissolvedItems().get(i), 1, 33422+i, 0);
											plr.getPA().sendString(33428+i, String.valueOf(Misc.formatCoins(plr.getRecentlyDissolvedPrices().get(i))));
										}
									}
									plr.getPA().sendString(33415, String.valueOf(Misc.formatCoins(plr.totalEarnedExchangePoints)));
									plr.getItems().sendItemContainer(33403, Lists.newArrayList(new GameItem(23, 1)));
									plr.getPA().sendInterfaceSet(33400, 33404);
									plr.getItems().sendInventoryInterface(33405);
									plr.getPA().sendFrame126("@gre@" + plr.exchangePoints, 33410);
									plr.getPA().sendFrame126("@red@0", 33409);
								}),
				...
				break;
				case 121315:
				sendNpcChat("Welcome to the TzHaar Foundry, I run this place!","Smelt your item's for some amazing points!");
				break;
	...
	public void sendOption5(String option, String s, String s1, String s2, String s3, String s4) {
		c.dialogueOptions = 5;
		c.getPA().sendFrame126(option, 2493);
		c.getPA().sendFrame126(s, 2494);
		c.getPA().sendFrame126(s1, 2495);
		c.getPA().sendFrame126(s2, 2496);
		c.getPA().sendFrame126(s3, 2497);
