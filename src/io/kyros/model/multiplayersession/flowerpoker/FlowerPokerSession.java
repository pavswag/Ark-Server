...
import io.kyros.model.multiplayersession.*;
import io.kyros.util.Misc;
import io.kyros.util.discord.Discord;
import io.kyros.util.logging.player.ItemFlowerPokerTradeLog;
            ...
            case MultiplayerSessionStage.CONFIRM_DECISION:
                ...
                if (stage.hasAttachment() && stage.getAttachment() != player) {
                    ...
                    for (Player h : items.keySet()) {
                        ...
                        sb.append(h.getDisplayName()).append(" is offering: ");
                        for (GameItem item : items.get(h)) {
                            pot.add(new GameItem(item.getId(), item.getAmount()));
                            sb.append(item.getDef().getName()).append(" x ").append(item.getAmount()).append(", ");
                        }
                    }
...
/*                    player.foundryPoints -= 2_000_000;
                    recipient.foundryPoints -= 2_000_000;*/

                    Discord.writeServerSyncMessage("[GAMBLE] " +sb.toString());
