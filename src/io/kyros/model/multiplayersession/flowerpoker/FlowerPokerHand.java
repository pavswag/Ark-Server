...
public class FlowerPokerHand {
    ...
    public static List<Position> positions = Arrays.asList(
            new Position(1776,3564, 0),
            new Position(1781, 3564, 0),
            new Position(1776, 3575, 0),
            new Position(1781, 3575, 0));
    ...
    private void displayWinner() {
        ...
        if (winner == otherHand) {
            ...
            giveItems(other);
            Discord.writeServerSyncMessage("[GAMBLE] " + other.getDisplayName() + " has won!");
        } else {
            player.forcedChat(handToString + totalPotMessage);
            other.forcedChat(otherHandToString + " - I LOSE");
            giveItems(player);
            Discord.writeServerSyncMessage("[GAMBLE] " + player.getDisplayName() + " has won!");
        }
    ...
    public long calculateFoePot() {
        ...
        for (GameItem item : prizePool) {
            if (item != null) {
                    long foeBurnRate = FireOfExchangeBurnPrice.getBurnPrice(null, item.getId(), false);
                if (foeBurnRate > 0) {
    ...
    public String calculateTotalPotMessage() {
        ...
        if ( foePot > 0 ){
            ...
            message += " [" + Misc.formatCoins(foePot) + " MP]";
        }
        return message;
    }
    /**
