package io.kyros.model.multiplayersession.duel;
...
import io.kyros.util.logging.player.ItemStakeLog;
...
/*				if (player.hasFollower || recipient.hasFollower) {
	...
	public void showRewardComponent(Player c) {
		...
		if (c.getOutStream() != null) {
			...
			c.getOutStream().writeInt(6822);
			c.getOutStream().writeShort(itemList.size());
		...
		players.stream().filter(Objects::nonNull).forEach(c -> {
			if (c.getOutStream() != null) {
				for (int i = 0; i < c.playerEquipment.length; i++) {
					...
					c.getOutStream().writeInt(13824);
					c.getOutStream().writeInt(i);
					c.getOutStream().writeShort(c.playerEquipment[i] + 1);
					if (c.playerEquipment[i] > -1) {
						if (c.playerEquipmentN[i] > 254) {
							c.getOutStream().writeByte(255);
							c.getOutStream().writeInt(c.playerEquipmentN[i]);
						} else {
