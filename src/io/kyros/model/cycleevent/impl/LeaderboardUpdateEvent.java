...
import io.kyros.content.leaderboards.*;
import io.kyros.model.cycleevent.Event;
import io.kyros.model.entity.player.PlayerHandler;
import io.kyros.sql.leaderboard.LeaderboardGetAll;
import io.kyros.sql.leaderboard.LeaderboardRewardAdd;
...
import static io.kyros.content.leaderboards.LeaderboardUtils.*;
...
/*				// If we just selected daily winners for sunday then we select the weekly winners.
				...
				PlayerHandler.addQueuedAction(() -> Server.getPlayers().stream().filter(Objects::nonNull).forEach(LeaderboardUtils::checkRewards));
			}
			return null;
		});
	}
	...
	private static void update(LocalDate now, LeaderboardPeriodicity period) {
		Preconditions.checkArgument(period == LeaderboardPeriodicity.TODAY);
				...
				if (keySet.getKey() == type) {
					for (LeaderboardEntry entry : keySet.getValue()) {
						...
						if (diff.getDays() == 0) {
                            ...
                            if (!dailyUpdate) {
                                if (!entry.getLocalDate().isEqual(mondayOfThisWeek)) {
                                    entry.getLocalDate();
                                }
                            }
                        }
		...
		Map<LeaderboardType, List<LeaderboardEntry>> sortedMap = daily;
		sortedMap.clear();
	...
	private static List<RewardEntry> getRewards(LeaderboardPeriodicity period) {
		...
		for (LeaderboardType type : LeaderboardType.values()) {
			List<LeaderboardEntry> entries = daily.get(type);
