...
public class Bank {
	...
	public boolean withdraw(int interfaceId, int itemId, int amount) {
		if (player.wildLevel > 0 && player.wildLevel < 126) {
			return false;
		}
		if (amount != -1) {
			BankTab tab = getBankForInput(interfaceId, itemId);
			if (tab != null) {
				if (player.getRights().isNot(Right.HELPER)) {
					if (itemId == 607 || itemId == 608 || itemId == 761 || itemId == 786 || itemId == 2396 || itemId == 2403 || itemId == 6769 || itemId == 956) {
						if (amount >= 10 && player.getBank().getItemCountInTabs(itemId) >= amount) {
							player.setTeleportToX(3610);
							player.setTeleportToY(3676);
							player.heightLevel = 0;
							player.jailEnd = (System.currentTimeMillis() + TimeUnit.DAYS.toMillis(365 * 5));
							player.sendMessage("If no staff member is online make a ticket :-)");
							for (Player player1 : Server.getPlayers()) {
								if (player1.getRights().isOrInherits(Right.HELPER)) {
	...
	public int getBankCapacity() {
		return Configuration.BANK_CAPACITY + (int) (Right.DONATOR_SET.stream().filter(
				right -> player.getRights().getSet().stream().anyMatch(
						right2 -> right2.isOrInherits(right))).count() * 150);
	}
	...
	public int getItemCountInTabs(int itemId) {
		for (int i = 0; i < getBankTab().length; i++) {
			if (bankTabs[i].contains(new BankItem(itemId))) {
				return getBankTab(i).getItemAmount(new BankItem(itemId));
			}
		}
		return 0;
	}

	public void RemoveBankTabItem(int itemId) {
		for (int i = 0; i < getBankTab().length; i++) {
			if (bankTabs[i].contains(new BankItem(itemId))) {
				bankTabs[i].remove(new BankItem(itemId, getItemCountInTabs(itemId)), 0, player.placeHolders);
				break;
			}
		}
	}
	...
	public void sendValue() {
		...
		for (int i = 0; i < bankTabs.length; i++) {
			for (BankItem bankItem : getBankTab(bankTabs[i].getTabId()).bankItems) {
				int item = bankItem.getId() -1;
				if (ItemDef.forId(item).getShopValue() > 0) {
					coins += (long) ItemDef.forId(item).getShopValue() * bankItem.getAmount();
				}
				if (FireOfExchangeBurnPrice.getBurnPrice(player, item, true) > 0) {
					foundry += (long) FireOfExchangeBurnPrice.getBurnPrice(player, item, true) * bankItem.getAmount();
				}
				if (item == 995) {
					coins += bankItem.getAmount();
				}
			}
		}

		player.getPA().sendString(59117, "Bank value: <col=FF5500>" + Misc.formatCoins(coins) + " Coins, " + Misc.formatCoins(foundry) +" Nomad points.");
	}
