...
public class Stream {
	...
	public void writeUnsignedByte(int i) {
		if (i < 0 || i > 255) {
			throw new IllegalArgumentException("Value must be between 0 and 255 for an unsigned byte.");
		}
		ensureCapacity(1);
		buffer[currentOffset++] = (byte) (i & 0xFF);  // Mask the int to fit in a byte (0 to 255 range)
	}

	public void writeShortSmart(int i) {
		ensureCapacity(1);
		writeByte(i > Byte.MAX_VALUE ? 100 : 128);
		if(i > Byte.MAX_VALUE) {
			ensureCapacity(2);
			writeShort(i + 32_768);
		} else {
	...
	public void writeBits(int numBits, int value) {
		if(!bitAccess)
			throw new RuntimeException("Trying to write bits without bit access");
	...
	public static int[] bitMaskOut = new int[]{0, 1, 3, 7, 15, 31, 63, 127, 255, 511, 1023, 2047, 4095, 8191, 16383, 32767, 65535, 131071, 262143, 524287, 1048575, 2097151, 4194303, 8388607, 16777215, 33554431, 67108863, 134217727, 268435455, 536870911, 1073741823, Integer.MAX_VALUE, -1};
