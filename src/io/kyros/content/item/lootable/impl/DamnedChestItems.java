...
public class DamnedChestItems implements Lootable {
...
public static void loadItems() {
        ...
        YamlLoader.LoadedBox loadedBox = YamlLoader.loadItems("DamnedChestItems");
...
rates = loadedBox.getRates();
    ...
    public static ArrayList<GameItem> getVeryRareDrops() {
        ArrayList<GameItem> drops = new ArrayList<>();
        List<GameItem> found = items.get(LootRarity.VERY_RARE);
        for(GameItem f : found) {
            boolean foundItem = false;
            for(GameItem drop : drops) {
                if (drop.getId() == f.getId()) {
