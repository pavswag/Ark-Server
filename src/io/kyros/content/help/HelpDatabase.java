...
public class HelpDatabase {
	 ...
	 */
	private static final HelpDatabase database = new HelpDatabase();

	/**
	 * A collection of all of the available requests in the game
	 */
	private final List<HelpRequest> requests = new ArrayList<>();
	...
	public void updateElements(Player player) {
		...
		for (int i = 0; i < requests.size(); i++) {
			...
			player.getPA().sendInterfaceHidden(0, 59571 + (i * 4));
		}
	}
