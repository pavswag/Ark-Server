...
import static io.kyros.content.dwarfleaguecannon.CannonConstants.ALLOWED_REV_AREAS;
import static io.kyros.content.dwarfleaguecannon.CannonConstants.CANNON_PIECES;
import static io.kyros.content.dwarfleaguecannon.CannonConstants.CANNON_SIZE;
import static io.kyros.content.dwarfleaguecannon.CannonConstants.COMPLETE_CANNON_OBJECT_ID;
import static io.kyros.content.dwarfleaguecannon.CannonConstants.MAX_DAMAGE;
import static io.kyros.content.dwarfleaguecannon.CannonConstants.MAX_GRANITE_DAMAGE;
import static io.kyros.content.dwarfleaguecannon.CannonConstants.PLACING_ANIMATION;
import static io.kyros.content.dwarfleaguecannon.CannonConstants.PROHIBITED_CANNON_AREAS;
import static io.kyros.content.dwarfleaguecannon.CannonConstants.PROJECTILE_ID;
...
public class Cannon {
    ...
    public static void attemptPlace(Player player) {
        if ((Arrays.stream(PROHIBITED_CANNON_AREAS).anyMatch(boundary -> boundary.in(player)) &&
        ...
        } else if (!io.kyros.content.dwarfleaguecannon.CannonRepository.hasDistanceFromOtherCannons(player.getPosition())) {
            ...
            player.setLeagueCannon(new io.kyros.content.dwarfleaguecannon.Cannon(player.getPosition()));
    ...
    public static boolean clickObject(Player player, int objectId, Position position, int option) {
        if (objectId == COMPLETE_CANNON_OBJECT_ID) {
            if (!io.kyros.content.dwarfleaguecannon.CannonRepository.exists(position)) {
    ...
    public static boolean clickItem(Player player, int itemId) {
        if (itemId == 26520) {
            io.kyros.content.dwarfleaguecannon.Cannon.attemptPlace(player);
            return true;
        }
     ...
     */
    private io.kyros.content.dwarfleaguecannon.CannonRotationState rotationState = io.kyros.content.dwarfleaguecannon.CannonRotationState.NORTH;
    ...
    public boolean equals(Object o) {
        ...
        io.kyros.content.dwarfleaguecannon.Cannon cannon = (io.kyros.content.dwarfleaguecannon.Cannon) o;
    ...
    private void place(Player player) {
        if (io.kyros.content.dwarfleaguecannon.CannonRepository.add(this)) {
...
/*        if (!operating && player.distance(getPosition()) <= 18 && tickCheck <= 0) {
    ...
    private void rotate() {
        int next = (rotationState.ordinal() + 1) % io.kyros.content.dwarfleaguecannon.CannonRotationState.values().length;
        rotationState = io.kyros.content.dwarfleaguecannon.CannonRotationState.values()[next];
    ...
    public List<NPC> getShootableNpcs(Player player) {
        ...
        int rotationStateIndex = (rotationState.ordinal() + 1) % io.kyros.content.dwarfleaguecannon.CannonRotationState.values().length;
        ...
        for (NPC local : possibleTargets) {
            ...
            switch (io.kyros.content.dwarfleaguecannon.CannonRotationState.values()[rotationStateIndex]) {
                case NORTH:
