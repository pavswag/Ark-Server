...
import io.kyros.util.logging.player.CompletedTobLog;

import java.util.List;

import static io.kyros.content.Jingles.COMPLETE_RAID;
...
public class Verzik extends TobBoss {
    ...
    private void openExit() {
        ...
        getInstance().getPlayers().forEach(plr -> {
            ...
            plr.getPA().sendJingle(COMPLETE_RAID);
                ...
                if (container.getTotalExecutions() == 9) {
                    ...
                    getInstance().getPlayers().forEach(plr -> {
                        ...
                        if (safeSpotList.stream().noneMatch(pos -> plr.getPosition().equals(pos))) {
                            plr.appendDamage(40 + Misc.trueRand(40), HitMask.HIT);
                        } else {
                            plr.appendDamage(0, HitMask.MISS);
                        }
                    });
                    container.stop();
                }
            ...
            public void execute(CycleEventContainer container) {
                getInstance().getPlayers().forEach(plr2 -> {
                    if (insideOf(plr2.getPosition())) {
                        ...
                        plr2.appendDamage(damage, damage > 0 ? HitMask.MISS : HitMask.HIT);
                    }
                });
    ...
    public void onDeath() {
        ...
        this.getInstance().getPlayers().forEach(p -> {
            p.sendMessage(message);

            for (TaskMasterKills taskMasterKills : p.getTaskMaster().taskMasterKillsList) {
                if (taskMasterKills.getDesc().equalsIgnoreCase("Theatre Of Blood")) {
                    taskMasterKills.incrementAmountKilled(1);
                    p.getTaskMaster().trackActivity(p, taskMasterKills);
                }
            }
        });
    }
