package io.kyros.content.minigames.tob.instance;
            ...
            if (player.getTobContainer().inTob()) {
                for (Player player1 : playerList) {
                    if (player.connectedFrom.equals(player1.connectedFrom)) {
                        player.sendMessage("You must be as dumb as dealer is, he tried this and failed.");
                        player1.sendMessage("You must be as dumb as dealer is, he tried this and failed.");
                        return;
                    }
                }
            }
        ...
        if (getPlayers().stream().allMatch(plr -> getPlayerRoomIndex(plr) == roomIndex)) {
            ...
            getPlayers().forEach(plr -> {
                if (lastRoom)
                    ...
                    plr.moveTo(new Position(currentRoom.getFightStartPosition().getX(), currentRoom.getFightStartPosition().getY(), plr.getHeight()));//Default back to the player's Height
                plr.getPA().closeAllWindows();
            });
            lastRoom = false;
        } else {
