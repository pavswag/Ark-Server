                ...
                if (tick == 2) {
                    if (ctx.ranger.lastDamageTaken > 0 || player.getInferno().glyph.isDead()) {
                        ...
                        if (Misc.random(500) + 200 > Misc.random(MagicMaxHit.mageDefence(player))) {
                            if (amount == 0) {
                                player.appendDamage(ctx.ranger, amount, HitMask.MISS);
                            } else {
                                player.appendDamage(ctx.ranger, amount, HitMask.HIT);
                            }
                        } else {
                            player.appendDamage(ctx.ranger, 0, HitMask.MISS);
                        }
                    } else {
                        amount = Misc.random(0, 70);
                        if (amount == 0) {
                            player.getInferno().glyph.appendDamage(ctx.ranger, amount, HitMask.MISS);
                        } else {
                            player.getInferno().glyph.appendDamage(ctx.ranger, amount, HitMask.HIT);
                        }
                    }
                    container.stop();
                }
            ...
            public void execute(CycleEventContainer container) {
                ...
                if (tick == 2) {
                    if (ctx.mager.lastDamageTaken > 0 || player.getInferno().glyph.isDead()) {
                        ...
                        if (Misc.random(500) + 200 > Misc.random(MagicMaxHit.mageDefence(player))) {
                            if (amount == 0) {
                                player.appendDamage(ctx.mager, amount, HitMask.MISS);
                            } else {
                                player.appendDamage(ctx.mager, amount, HitMask.HIT);
                            }
                        } else {
                            player.appendDamage(ctx.mager, 0, HitMask.MISS);
                        }
                    } else {
                        amount = Misc.random(0, 70);
                        if (amount == 0) {
                            player.getInferno().glyph.appendDamage(ctx.mager, amount, HitMask.MISS);
                        } else {
                            player.getInferno().glyph.appendDamage(ctx.mager, amount, HitMask.HIT);
                        }
                    }
                    container.stop();
                }
