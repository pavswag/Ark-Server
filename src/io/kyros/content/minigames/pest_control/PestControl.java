...
public class PestControl {
	...
	public static void createNewGame() {
		killAllNpcs();
		getPlayers().forEachFiltered(player -> Boundary.isIn(player, LOBBY_BOUNDARY), PestControl::addPlayerToGame);
		finalizeLobby();
		spawnAllNpcs();
		gameTime = GAME_TIME;
		gameActive = true;
	...
	public static void killAllNpcs() {
		for (int i = 0; i < PORTAL_DATA.length; i++) {
			...
			if (npc != null) {
                Server.getNpcs().get(npc.getIndex()).applyDead = true;
                Server.getNpcs().get(npc.getIndex()).setUpdateRequired(true);
                getNpcs().remove(npc);
			}
		}
	}
