...
import io.kyros.content.minigames.bounty_hunter.events.TargetDelay;
import io.kyros.content.minigames.bounty_hunter.events.TargetSelector;
...
public class BountyHunter extends CycleEvent {
	 ...
	 */
	private final TargetDelay targetDelay = new TargetDelay(this);
	 ...
	 */
	public int outsideBoundsTicks = INITIAL_BOUNDARY_TICKS;
	...
	public void execute(CycleEventContainer container) {
		...
		if (hasTarget()) {
			Player target = PlayerHandler.getPlayerByLoginName(this.target.getLoginName());
			if (Objects.isNull(target) || target.isDisconnected()) {
		...
		if (!CycleEventHandler.getSingleton().isAlive(targetDelay)) {
			if (warnings >= MAXIMUM_WARNINGS) {
				...
				delayedTargetTicks = KILLED_TARGET_SELECTOR_DELAY;
	...
	public void updateSkull() {
		skull = BountyHunterSkull.getSkull(ItemsLostOnDeath.calculateValueOfLostItems(player));
	}
	...
	public void updateTargetUI() {
		updateStatisticsUI();
		player.getPA().sendInterfaceHidden(/*statisticsVisible ? 0 : 1*/1, 28020);
		if (target == null) {
			...
			player.getPA().sendInterfaceHidden(0, 28030);
			for (int i = 28032; i <= 28040; i += 2) {
				player.getPA().sendInterfaceHidden(1, i);
			}
			return;
		}
		Player playerTarget = PlayerHandler.getPlayerByLoginName(target.getLoginName());
		...
		updateSkull();
		player.getPA().sendFrame126(playerTarget.getDisplayName(), 28004);
		...
		for (int i = 28030; i <= 28040; i += 2) {
			if (i == playerTarget.getBH().skull.getFrameId()) {
				player.getPA().sendInterfaceHidden(0, i);
			} else {
				player.getPA().sendInterfaceHidden(1, i);
			}
		}
	...
	public void teleportToTarget() {
		if (player.getPosition().inClanWars()) {
			player.sendMessage("@cr10@You can not do this here.");
			return;
		}
		if (!player.getPosition().inWild()) {
			player.sendMessage("You must be in the wilderness to use this.");
			return;
		}
		if (!spellAccessible) {
		...
		if (System.currentTimeMillis() - lastTeleport < 300_000) {
			player.sendMessage("You can only use this spell every 5 minutes.");
			return;
		}
		...
		if (!target.getPosition().inWild()) {
			player.sendMessage("Your target is not in the wilderness, they must be to be teleported to.");
			return;
		}
		if (Boundary.isIn(target, new Boundary(3187, 3958, 3194, 3962))) {
			player.sendMessage("You can't teleport to this player, they're in the afk area.");
			return;
		}
		if (player.playerAttackingIndex > 0 && target.getIndex() == player.playerAttackingIndex) {
			player.sendMessage("You cannot use this spell whilst in combat with your target.");
			return;
		}
		int targetX = target.getX();
		int targetY = target.getY();
				if (!player.getRegionProvider().get(targetX, targetY).canMove(targetX, targetY, targetX + 1, targetY + 1, target.heightLevel, 1, 1)) {
					player.sendMessage("Your target is in a blocked in area, you cannot teleport to them right now.");
					return;
				}
			
		
		player.getPA().movePlayer(targetX, targetY, target.heightLevel);
		lastTeleport = System.currentTimeMillis();
	}

	public boolean isTarget(Player other) {
		return hasTarget() && getTarget().getLoginName().equalsIgnoreCase(other.getLoginName());
	}

	/**
	 * Checks if the player is the current target
	 * @param playerAttackingIndex the index of the player we're attacking
	 * @return true if this is our target
	 */
	public boolean isTarget(int playerAttackingIndex) {
        Player player = getPlayers().get(playerAttackingIndex);
		return player != null && hasTarget() && getTarget().getLoginName() != null
				&& getTarget().getLoginName().equalsIgnoreCase(player.getLoginName());
	}
