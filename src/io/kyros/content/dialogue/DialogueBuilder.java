...
import io.kyros.content.dialogue.types.ItemStatementDialogue;
import io.kyros.content.dialogue.types.MakeItemDialogue;
import io.kyros.content.dialogue.types.NpcDialogue;
import io.kyros.content.dialogue.types.OptionDialogue;
import io.kyros.content.dialogue.types.PlayerDialogue;
import io.kyros.content.dialogue.types.StatementDialogue;
...
/**
 * Builds a list of DialogueObjects.
 */
public class DialogueBuilder {

    public static final DialogueOption EXIT_DIALOGUE_OPTION = new DialogueOption("Exit", plr -> plr.getPA().closeAllWindows());
    ...
    public DialogueBuilder npc(int npcId, LuaValue luaText) {
        String[] text = new String[luaText.length()];
        for (int i = 0; i < luaText.length(); i++) {
            text[i] = luaText.get(i + 1).tojstring();
        }
    ...
    public DialogueBuilder option(List<DialogueOption> options) {
        return option("Select Option", options.toArray(new DialogueOption[0]));
    }
    ...
    public DialogueBuilder itemStatement(int itemId, String...text) {
        return itemStatement(true, itemId, 150, ItemDef.forId(itemId).getName(), text);
    }
