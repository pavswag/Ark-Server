...
import io.kyros.util.logging.player.RollDiceLog;
...
/*		if (!inDicingArea(c)) {
		...
		if (System.currentTimeMillis() - c.diceDelay >= 3000) {
			...
			Server.getPlayers().nonNullStream()
					.filter(player -> player.getPosition().withinDistance(c.getPosition(), 15) && player.heightLevel == 0)
					.forEach(player -> {
