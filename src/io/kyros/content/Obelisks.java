...
public class Obelisks {
	 ...
	 */
	private static final Obelisks INSTANCE = new Obelisks();
	...
	static final class Event extends CycleEvent {
		...
		public void execute(CycleEventContainer container) {
			...
			List<Player> players = Server.getPlayers().nonNullStream().filter(Objects::nonNull).filter(player -> Boundary.isIn(player, boundary)).collect(Collectors.toList());
			if (players.size() > 0) {
