...
import io.kyros.util.logging.global.DailyRewardsCompletedLog;
import io.kyros.util.logging.player.DailyRewardLog;
...
public class DailyRewards {
    ...
    public void onLogin() {
        ...
        if (streak == rewards.size()) {
            lastRewardIdentifier = null;
            System.out.println("Reset daily rewards, due to reaching max streak.");
        }
    }
    ...
    public void claim() {
        boolean developer = Server.isDebug() && player.getRights().contains(Right.STAFF_MANAGER);
        long minutesUntilClaim = getMinutesUntilClaim();
