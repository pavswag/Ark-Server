...
import io.kyros.content.commands.punishment.PunishmentCommandArgs;
import io.kyros.content.commands.punishment.PunishmentCommandParser;
...
import static io.kyros.punishments.PunishmentType.MAC_BAN;
import static io.kyros.punishments.PunishmentType.NET_BAN;
...
public class Netban implements PunishmentCommandParser {
    ...
    public void add(Player staff, PunishmentCommandArgs args) {
        ...
        Server.getPlayers().nonNullStream().filter(it -> addresses.equals(it.getValidAddresses()) && it.getRights().isNot(Right.STAFF_MANAGER)).forEach(Player::forceLogout);
