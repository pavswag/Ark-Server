...
public class WorldEventContainer {
    ...
    private static final int CYCLES_BETWEEN_EVENTS = Misc.toCycles(30, TimeUnit.MINUTES);
    ...
    public static final List<WorldEvent> WORLD_EVENT_LIST =
            Collections.unmodifiableList(Lists.newArrayList(/*new WGWorldEvent(),*/ new TournamentWorldEvent(), new HesporiWorldEvent(),
                    new WildernessBossWorldEvent()));
    ...
    private void next() {
        ...
        worldEvent.announce(Server.getPlayers());
        Server.getPlayers().forEach(p -> p.getQuestTab().updateInformationTab());
        scheduleNext();
    }
    ...
    public List<String> getWorldEventStatuses() {
        ...
        if (event.isPresent()) {
            ...
            /*if (!event.get().isWG()) {
                stringList.add(getNextStatusString(new WGWorldEvent()));
            }*/
        } else {
            stringList.add(getNextStatusString(getNextEvent()));
            if (!getNextEvent().isOutlast()) {
