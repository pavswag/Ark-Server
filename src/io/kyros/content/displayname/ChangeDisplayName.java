package io.kyros.content.displayname;
...
import io.kyros.sql.displayname.CheckDisplayNameNotTakenSqlQuery;
import io.kyros.sql.displayname.SetDisplayNameSqlQuery;
import io.kyros.sql.etc.HiscoresUpdateQuery;
import io.kyros.util.logging.player.ChangeDisplayNameLog;
...
public class ChangeDisplayName {
    ...
    private static void start(Player player) {
        ...
        new DialogueBuilder(player).setNpcId(Npcs.ACCOUNT_GUIDE)
                ...
                .npc("@red@WARNING@bla@: when donating, and voting", "enter your @blu@display name@bla@!")
            ...
            try {
                ...
                if (!successful) {
                    ...
                    player.addQueuedAction(plr -> {
                        ...
                        Server.getPlayers().nonNullStream().forEach(it -> it.getFriendsList().updateDisplayName(player, oldDisplayName));
                    });
                }
            } finally {
