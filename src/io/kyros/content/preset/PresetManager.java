package io.kyros.content.preset;
...
import static io.kyros.content.CompletionistCape.hasRequirements;
...
public class PresetManager {
	...
	public void init() {
		try {
			...
			JsonElement obj = parser.parse(new FileReader(file));

			if (!obj.isJsonArray()) {
				// Handle the case where the file is empty or not valid JSON
				System.out.println("Invalid JSON format in default_presets.json");
				return;
			}

			JsonArray jsonUpdates = obj.getAsJsonArray();

			Type listType = new TypeToken<ArrayList<DefaultPreset>>() {}.getType();
			...
			listType = new TypeToken<ArrayList<Preset>>() {}.getType();
			...
			for (File f : files) {
				if (f.exists()) {
					...
					if (!obj.isJsonArray()) {
						// Handle the case where the file is empty or not valid JSON
						System.out.println("Invalid JSON format in " + f.getName());
						continue;
					}

					JsonArray jsonUpdate = obj.getAsJsonArray();
			...
			e.printStackTrace();
			System.out.println("Error loading presets!");
	...
	public void open(Player player) {
		...
		if (player.isBoundaryRestricted()) {
			return;
		}
	...
	private void loadPreset(Player player, boolean loadPrevious) {
		...
		if (player.getPosition().inWild()) {
			player.sendMessage("You can't do this in the wilderness fog!");
			return;
		}
		...
		try {
			...
			if (!defaultPreset) {
				...
				for (SkillLevel requirement : ItemDef.forId(preset.getEquipment().getHelmet()).getRequirements()) {
				...
				for (SkillLevel requirement : ItemDef.forId(preset.getEquipment().getLegs()).getRequirements()) {
					if (player.getLevelForXP(player.playerXP[requirement.getSkill().getId()]) < requirement.getLevel()) {
						player.sendMessage("You need " + Misc.anOrA(requirement.getSkill().toString()) + " " + requirement.getSkill().toString()
								+ " level of " + requirement.getLevel() + " to wear this item.");
						return;
				...
				for (SkillLevel requirement : ItemDef.forId(preset.getEquipment().getShield()).getRequirements()) {
					if (player.getLevelForXP(player.playerXP[requirement.getSkill().getId()]) < requirement.getLevel()) {
						player.sendMessage("You need " + Misc.anOrA(requirement.getSkill().toString()) + " " + requirement.getSkill().toString()
								+ " level of " + requirement.getLevel() + " to wear this item.");
						return;
				...
				for (SkillLevel requirement : ItemDef.forId(preset.getEquipment().getWeapon()).getRequirements()) {
					if (player.getLevelForXP(player.playerXP[requirement.getSkill().getId()]) < requirement.getLevel()) {
						player.sendMessage("You need " + Misc.anOrA(requirement.getSkill().toString()) + " " + requirement.getSkill().toString()
								+ " level of " + requirement.getLevel() + " to wear this item.");
						return;
				...
				for (SkillLevel requirement : ItemDef.forId(preset.getEquipment().getRing()).getRequirements()) {
					if (player.getLevelForXP(player.playerXP[requirement.getSkill().getId()]) < requirement.getLevel()) {
						player.sendMessage("You need " + Misc.anOrA(requirement.getSkill().toString()) + " " + requirement.getSkill().toString()
								+ " level of " + requirement.getLevel() + " to wear this item.");
						return;
				...
				for (SkillLevel requirement : ItemDef.forId(preset.getEquipment().getBoots()).getRequirements()) {
					if (player.getLevelForXP(player.playerXP[requirement.getSkill().getId()]) < requirement.getLevel()) {
						player.sendMessage("You need " + Misc.anOrA(requirement.getSkill().toString()) + " " + requirement.getSkill().toString()
								+ " level of " + requirement.getLevel() + " to wear this item.");
						return;
				...
				for (SkillLevel requirement : ItemDef.forId(preset.getEquipment().getGloves()).getRequirements()) {
					if (player.getLevelForXP(player.playerXP[requirement.getSkill().getId()]) < requirement.getLevel()) {
						player.sendMessage("You need " + Misc.anOrA(requirement.getSkill().toString()) + " " + requirement.getSkill().toString()
								+ " level of " + requirement.getLevel() + " to wear this item.");
						return;
				...
				for (SkillLevel requirement : ItemDef.forId(preset.getEquipment().getPlate()).getRequirements()) {
					if (player.getLevelForXP(player.playerXP[requirement.getSkill().getId()]) < requirement.getLevel()) {
						player.sendMessage("You need " + Misc.anOrA(requirement.getSkill().toString()) + " " + requirement.getSkill().toString()
								+ " level of " + requirement.getLevel() + " to wear this item.");
						return;
				...
				for (SkillLevel requirement : ItemDef.forId(preset.getEquipment().getCape()).getRequirements()) {
					if (player.getLevelForXP(player.playerXP[requirement.getSkill().getId()]) < requirement.getLevel()) {
						player.sendMessage("You need " + Misc.anOrA(requirement.getSkill().toString()) + " " + requirement.getSkill().toString()
								+ " level of " + requirement.getLevel() + " to wear this item.");
						return;
				...
				for (int i : new int[]{20760, 13280, 13329, 21186, 13337, 13069, 24855, 13331, 13333, 13335, 21776, 21780, 21784, 21898, 21285, 21284, 27363, 28092}) {
					if (i == preset.getEquipment().getCape() && !player.maxRequirements(player)) {
						player.sendMessage("You must have maxed out all skills in order to wear the max cape.");
						return;
					}
				}

				if (preset.getEquipment().getCape() == Items.COMPLETIONIST_CAPE && !hasRequirements(player)) {
					player.sendMessage("@red@Completionist cape cannot be worn as you do not meet the requirements!.");
					return;
				}
				for (SkillLevel requirement : ItemDef.forId(preset.getEquipment().getNecklace()).getRequirements()) {
					if (player.getLevelForXP(player.playerXP[requirement.getSkill().getId()]) < requirement.getLevel()) {
						player.sendMessage("You need " + Misc.anOrA(requirement.getSkill().toString()) + " " + requirement.getSkill().toString()
								+ " level of " + requirement.getLevel() + " to wear this item.");
						return;
				...
				boolean wearing2h = player.getItems().is2handed(ItemAssistant.getItemName(player.playerEquipment[Player.playerWeapon]).toLowerCase(), player.playerEquipment[Player.playerWeapon]);
				if (wearing2h) {
				...
				if (player.getMode().isIronmanType() || player.getMode().equals(Mode.forType(ModeType.WILDYMAN)) || player.getMode().equals(Mode.forType(ModeType.HARDCORE_WILDYMAN))) {
					player.sendMessage("you're not permitted to purchase default presets.");
					return;
				}
	...
	public void loadLastPreset(Player player) {
		...
		if (Boundary.isIn(player, new Boundary(1664, 4224, 1727, 4287))) {
			return;
		}
		...
		if (player.isInTradingPost() || player.inTrade || player.isFping() || player.inDuel) {
			player.sendMessage("Please finish what you are doing.");
			return;
		}
		if (player.amDonated < 100 && player.wildLevel <= 0) {
		...
		if (Boundary.isIn(player, new Boundary(2525, 4708, 2551, 4726))){
			return;
		}
		if (player.teleTimer > 0) {
			return;
		...
		if (player.getY() >= 3520 && player.getY() <= 3525) {
			return;
		}
