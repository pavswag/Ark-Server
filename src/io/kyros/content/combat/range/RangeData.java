package io.kyros.content.combat.range;
...
import io.kyros.content.combat.effects.damageeffect.impl.bolts.*;
...
public class RangeData {
	...
	public static final int[] BOWS = { 19481, 26712, 19478, 12788, 9185, 11785, 21012, 26374, 33206, 26269, 33435, 839, 845, 847, 851, 855, 859, 841, 843, 849,
			853, 857, 12424, 861, 4212, 4214, 4215, 12765, 12766, 12767, 12768, 11235, 4216, 4217, 4218, 4219, 4220, 27655, 27610,
			4221, 4222, 4223, 4734, 6724, 20997, 21902, 22550, 25916, 25918, 26486, 25865, 25867, 25884, 25886, 25888, 33058, 22333, 33207, 33434, 25890, 25892,
			25894, 25896, 33160, 28540, 29599,
			//corrupted bow
	...
	public static final int[] CRYSTAL_BOWS = { 4212, 4214, 4215, 4216, 4217, 4218, 4219, 4220, 4221, 4222, 4223,
			//bofa
			25865, 25867, 25884, 25886, 25888, 25890, 25892, 25894, 25896, 33058, 33207, 22550, 25955, 33434,
			//corrupted bow
			23855, 23857, 23902, 29599,
	};

	public static final int[] NO_ARROW_DROP = { 11959, 10033, 10034, 4212, 22550, 27655, 4214, 4215, 4216, 4217, 4218, 4219, 4220, 4221,
			4222, 4223, 4734, 4934, 4935, 4936, 4937, 25865, 25867, 25884, 25886, 25888, 33058, 33207, 25890, 25892, 25894, 25896, 29599, 33434,
			//corrupted bow
			23855, 23857, 23902 };

	public static final int[] DOUBLE_SHOT_BOWS = {11235, 12765 , 12766, 12767, 12768, 29599, 33434};

	public static final int[] OTHER_RANGE_WEAPONS = { 11959, 10501, 10033, 10034, 800, 801, 802, 803, 804, 805, 20849, 806, 807, 808,
			...
			6522, 11230, 11231, 11233,11234, 22804, 22634, 21318, 19484, 22636, 28919, 28922};

	public static final int[] JAVELINS = { 825, 826, 827, 828, 829, 830, 21318, 19484, 22636 };
	...
	public static void fireProjectileNPCtoPLAYER(NPC player, Player target, int angle, int speed, int projectile, int startHeight, int endHeight, int time, int slope) {
		int playerX = player.getX();
		int playerY = player.getY();
		int npcX = target.getX();
		int npcY = target.getY();
		int xOffset = (playerY - npcY) * -1;
		int yOffset = (playerX - npcX) * -1;
		target.getPA().createPlayersProjectile2(playerX, playerY, xOffset, yOffset, angle, speed, projectile, startHeight, endHeight, target.getIndex(), time, slope);
	}
	...
	public static int getRangeStartGFX(Player c) {
		if(c.playerEquipment[Player.playerArrows] == 33411 || c.playerEquipment[Player.playerArrows] == 33423) {
			return 473;
		}
...
//				DARTS
				{ 806, 232 }, { 807, 233 }, { 808, 234 }, { 809, 235 }, { 810, 236 }, { 811, 237 }, { 11230, 237 }, {10501, 860},
...
//				BOFA
				...
				{ 25894, 1935 }, //cyan
				{ 25888, 1929 }, //purple
				{ 33058, 500 }, //Mage Spell? fuck it
				{ 22333, 19 },//Starter bow Consumes no arrows
				{ 27655, 2283},
//				ARROWS
	...
	public static int getRangeProjectileGFX(Player c) {
		...
		// Blowpipe
		if (c.getItems().isWearingItem(12926) || c.getItems().isWearingItem(28688)) {
		...
		// Crossbows
		if (c.playerEquipment[Player.playerWeapon] == 9185 || c.playerEquipment[Player.playerWeapon] == 26486 || c.playerEquipment[Player.playerWeapon] == 11785 || c.playerEquipment[Player.playerWeapon] == 21012
				|| c.playerEquipment[Player.playerWeapon] == 21902 || c.playerEquipment[Player.playerWeapon] == 26374 || c.playerEquipment[Player.playerWeapon] == 33206 || c.playerEquipment[Player.playerWeapon] == 25916
				|| c.playerEquipment[Player.playerWeapon] == 25918 || c.playerEquipment[Player.playerWeapon] == 8880 || c.playerEquipment[Player.playerWeapon] == 26269 || c.playerEquipment[Player.playerWeapon] == 33435) {
			return 27;
		}
		...
		if (c.playerEquipment[Player.playerWeapon] == 19481 || c.playerEquipment[Player.playerWeapon] == 26712 || c.playerEquipment[Player.playerWeapon] == 19478) {
		...
		int[][] data = {
				...
				// DARTS
				{ 806, 226 }, { 807, 227 }, { 808, 228 }, { 809, 229 }, { 810, 230 }, { 811, 231 }, { 11230, 231 },{ 10501, 860 },

				// JAVELINS
				{ 825, 200 }, { 826, 201 }, { 827, 202 }, { 828, 303 }, { 829, 504 }, { 830, 705 }, { 21318, 1386 }, { 19484, 1301 },{ 22636, 1620 },
				...
				{ 25888, 1928 }, //purple
				{ 33058, 500 }, //Mage Spell? fuck it
				{ 33207, 660 }, //Mage Spell? fuck it
				{ 33434, 2683 }, //Mage Spell? fuck it
	...
	public static int getRangeEndGFX(Player c, boolean weapon) {
		...
		int str = -1;
		int gfx = 0;
		int[][] data = { { 10033, 157, 100 }, { 10034, 157, 100 }, { 11959, 157, 100 }, {28922, 2731, 100} };
		for (int l = 0; l < data.length; l++) {
			if (itemId == data[l][0]) {
				str = data[l][1];
				gfx = data[l][2];
	...
	public static int getProjectileShowDelay(Player c, boolean weapon) {
		...
		Optional<Integer> original = PoisonedWeapon.getOriginal(itemId);
		int ammo = original.orElse(itemId);
		int[] data = { 806, 806, 808, 809, 810, 811, 10033, 10034, 11959, 11230, 10501 };
		int str = 53;
	...
	public static boolean boltSpecialAvailable(Player player, int...bolt) {
		if(!RangeData.wearingCrossbow(player))
			return false;
		...
		if (player.usingZaryteSpec && player.playerEquipment[Player.playerArrows] == optionalBolt.getAsInt()) {
			player.usingZaryteSpec = false;
			return true;
		}
	...
	public static boolean wearingCrossbow(Player player) {
		return player.playerEquipment[Player.playerWeapon] == 11785 || player.playerEquipment[Player.playerWeapon] == 9185
				...
				|| player.playerEquipment[Player.playerWeapon] == 26374 || player.playerEquipment[Player.playerWeapon] == 25916
				|| player.playerEquipment[Player.playerWeapon] == 25918 || player.playerEquipment[Player.playerWeapon] == 26486
				|| player.playerEquipment[Player.playerWeapon] == 8880 || player.playerEquipment[Player.playerWeapon] == 33206
				|| player.playerEquipment[Player.playerWeapon] == 26269 || player.playerEquipment[Player.playerWeapon] == 33435;
	}

	public static boolean wearingBolt(Player player) {
		if(player.playerEquipment[Player.playerArrows] == 33411 || player.playerEquipment[Player.playerArrows] == 33423) {
			return true;
		}
		int arrows = player.playerEquipment[Player.playerArrows];
		if (player.playerEquipment[Player.playerWeapon] == 8880) {
			if (arrows == 8882) {
				return true;
			}
		}
