...
public class NPCAutoAttack {
        ...
        return new Function<NPCCombatAttack, List<Player>>() {
            ...
            public List<Player> apply(NPCCombatAttack npcCombatAttack) {
                ...
                for (Player player : Server.getPlayers().toPlayerArray()) {
                    if (player != null && player.getInstance() == npc.getInstance() && !player.isIdle
    ...
    public static Function<NPCCombatAttack, List<Player>> getDefaultSelectPlayersForAttack32x32() {
        return new Function<NPCCombatAttack, List<Player>>() {
            @Override
            public List<Player> apply(NPCCombatAttack npcCombatAttack) {
                NPC npc = npcCombatAttack.getNpc();
                List<Player> players = Lists.newArrayList();
                for (Player player : Server.getPlayers().toPlayerArray()) {
                    if (player != null && npc.getDistance(player.getX(), player.getY()) <= 32 && !player.isIdle
                    && player.getHeight() == npc.getHeight()) {
                        players.add(player);
                    }
                }
    ...
    public NPCAutoAttack(Animation animation, ProjectileBase projectile, Graphic startGraphic, Graphic endGraphic, CombatType combatType,
                         int hitDelay, int attackDelay, int minHit, int maxHit, int poisonDamage, int distanceRequiredForAttack, boolean ignoreProjectileClipping,
