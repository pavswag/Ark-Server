...
import io.kyros.model.cycleevent.impl.NightmareStaffEvent;
...
public class VolatileNightmareStaff extends Special {
    ...
    public void hit(Player player, Entity target, Damage damage) {
        if (target instanceof Player) {
            player.usingMagic = true;
                if (((Player) target).specAmount >= 7.5) {
                    ((Player) target).specAmount -= 7.5;
                    ((Player) target).getPA().requestUpdates();
                } else if (((Player) target).specAmount < 7.5)
