    ...
    public static boolean check(Player c, Entity targetEntity, boolean sendMessages) {
        ...
        if (CastleWarsLobby.isInCw(c)) {
            if (targetEntity.isPlayer()) {
                Player c2 = targetEntity.asPlayer();
                if (c.getItems().isWearingItem(4514) && c2.getItems().isWearingItem(4514)) {
                    c.sendMessage("You cannot attack your own team member.");
                    return false;
                } else if (c.getItems().isWearingItem(4516) && c2.getItems().isWearingItem(4516)) {
                    c.sendMessage("You cannot attack your own team member.");
                    return false;
                } else {
                    return true;
