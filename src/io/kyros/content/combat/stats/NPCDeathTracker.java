...
public class NPCDeathTracker {
	...
	public int getKc(String name) {
		...
		if (name.equalsIgnoreCase("Abrograve Swamp")) {
			return player.arboCompletions;
		}

		if (name.equalsIgnoreCase("Grotesque Guardians")) {
			return tracker.getOrDefault("Dusk", 0);
		}
		if (name.equalsIgnoreCase("Perkfinder Minigame")) {
			int KC = 0;
			KC += tracker.getOrDefault("arzinian avatar of ranging", 0);
			KC += tracker.getOrDefault("arzinian avatar of strength", 0);
			KC += tracker.getOrDefault("arzinian avatar of magic", 0);
	...
	public void add(String name, int combatLevel, int bossPoints) {
		if (name == null) {
			...
			player.getActivityTracker().incrementBossKill(name);
