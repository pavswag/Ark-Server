...
public class CombatSpellData {
	...
	public static final int[] ANCIENT_AUTOCAST_STAFFS = {
			...
			22296, 24422, 24424, 24425, 8841, 33149, 21198, 33205, 33433, 27679, 22335, 28796,29594,84
	};
	...
	private static final Set<Integer> BOLT_SPELLS = Set.of(1160, 1163, 1166, 1169);
	private static final Set<Integer> SMOKE_SPELLS = Set.of(12939, 12963, 12951, 12975);
	private static final Set<Integer> SHADOW_SPELLS = Set.of(12987, 13011, 12999, 13023);
	private static final Set<Integer> BLOOD_SPELLS = Set.of(12901, 12919, 12911, 12929);
	private static final Set<Integer> ICY_SPELLS = Set.of(12861, 12881, 12871, 12891);

	public static boolean isBoltSpell(int spell) {
		return BOLT_SPELLS.contains(spell);
	}

	public static boolean isSmokeSpell(int spell) {
		return SMOKE_SPELLS.contains(spell);
	}

	public static boolean isShadowSpell(int spell) {
		return SHADOW_SPELLS.contains(spell);
	}

	public static boolean isBloodSpell(int spell) {
		return BLOOD_SPELLS.contains(spell);
	}

	public static boolean isIcySpell(int spell) {
		return ICY_SPELLS.contains(spell);
	}

	private static final Map<String, Set<Integer>> SPELL_TYPES = Map.of(
			"wind", Set.of(1158, 1161, 1167, 1172),
			"water", Set.of(1154, 1160, 1166, 1171),
			"earth", Set.of(1153, 1157, 1164, 1175),
			"fire", Set.of(1155, 1163, 1169, 1177),
			"blood", Set.of(12901, 12919, 12911, 12929),
			"smoke", Set.of(12939, 12963, 12951, 12975),
			"ice", Set.of(12861, 12881, 12871, 12891),
			"shadow", Set.of(12987, 13011, 12999, 13023)
	);

	private static final Map<String, Set<Integer>> SPELL_TIERS = Map.of(
			"strike", Set.of(1158, 1154, 1153, 1155, 12901, 12939, 12861, 12987),
			"bolt", Set.of(1161, 1160, 1157, 1163, 12919, 12963, 12881, 13011),
			"blast", Set.of(1167, 1166, 1164, 1169, 12911, 12951, 12871, 12999),
			"surge", Set.of(1172, 1171, 1175, 1177, 12929, 12975, 12891, 13023)
	);

	public static boolean isSpellOfType(int spell, String type) {
		return SPELL_TYPES.getOrDefault(type.toLowerCase(), Set.of()).contains(spell);
	}

	public static boolean isSpellOfTier(int spell, String tier) {
		return SPELL_TIERS.getOrDefault(tier.toLowerCase(), Set.of()).contains(spell);
	}

	public static boolean isSpecificSpell(int spell, String type, String tier) {
		return isSpellOfType(spell, type) && isSpellOfTier(spell, tier);
	}
	...
	public final static int[][] MAGIC_SPELLS = {
		...
		{ -1, 75, 1167, 1540, 1539, 1541, 35, 2, 0, 0, 0, 0, 0, 0, 0, 0 }, // Sanguinesti staff spell 98 (other ids are off 1)
			...
			{ -1, 99, 10853, 2322, 2324, 2324, 45, 2, 0, 0, 0, 0, 0, 0, 0, 0 }, // Staff Of Ambition - 102

			{ -1, 99, 11055, 0, 0, 2684, 45, 2, 0, 0, 0, 0, 0, 0, 0, 0 }, // Purging Staff - 103

			{ -1, 75, 11055, 2810, 0, 2731, 45, 2, 0, 0, 0, 0, 0, 0, 0, 0 }, // Wraith Staff - 104

			{ -1, 1, 11055, 2665, 2683, 2791, 200, 2, 0, 0, 0, 0, 0, 0, 0, 0 }, // The Stick of Last Resort - 105

			{ -1, 1, 7516, 3001, 3002, 3003, 200, 2, 0, 0, 0, 0, 0, 0, 0, 0 }, // BONK - 106
			{ -1, 1, 1057, 1811, 1744, 1813, 200, 2, 0, 0, 0, 0, 0, 0, 0, 0 }, // ALE OF THE SCOTTY - 107
	...
	public static int getEndGfxHeight(Player c) {
		...
		switch (MAGIC_SPELLS[c.oldSpellId][5]) {
			case 3003:
			case 1813:
				return 0;
		}
