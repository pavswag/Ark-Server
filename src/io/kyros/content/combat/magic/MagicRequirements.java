...
public class MagicRequirements extends MagicConfig {
	...
	public static boolean wearingStaff(Player c, int runeId) {
		...
		switch (runeId) {
			case Items.FIRE_RUNE:
				if (wearingTomeOfFire(c) || wep == 12000 || wep == 3053 || wep == 1387 || wep == 1393 || wep == 1401
						|| wep == 12796 || wep == 11789 || wep == 12795 || wep == 22335 || wep == 21198)
					return true;
				break;
			case Items.WATER_RUNE:
				if (wep == 1383 || wep == 1395 || wep == 12796 || wep == 21006 || wep == 11789 || wep == 6563 || wep == 1403 || wep == 20730  || wep == 12795)
					return true;
				break;
			case Items.AIR_RUNE:
				if (wep == 12000 ||wep == 1381 || wep == 1397 || wep == 1405 || wep == 20736 || wep == 20730 || wep == 22335)
					return true;
				break;

			case Items.EARTH_RUNE:
				if (wep == 1385 || wep == 1399 || wep == 1407 || wep == 6563 || wep == 20736 || wep == 21198)
					return true;
				break;
	...
	public static boolean checkMagicReqs(Player c, int spell, boolean deleteRunes) {
		if (spell > -1) {
			...
			if (!Boundary.isIn(c, Boundary.FOUNTAIN_OF_RUNE_BOUNDARY)) {
				...
				if (spell == 29 && c.playerEquipment[Player.playerWeapon] == 24144) {
					hasStaff1 = true;
					hasStaff2 = true;
					hasStaff3 = true;
					hasStaff4 = true;
				...
				if (spell == 47 && c.getItems().getInventoryCount(24607) > 0) {
					//ice barrage
					hasStaff1 = true;
					hasStaff2 = true;
					hasStaff3 = true;
				...
				if (spell == 55 && c.getItems().getInventoryCount(24621) > 0) {
					//Vengence
					hasStaff1 = true;
					hasStaff2 = true;
					hasStaff3 = true;
				...
				if (spell == 22 && c.getItems().getInventoryCount(24609) > 0) {
					//bind
					hasStaff1 = true;
					hasStaff2 = true;
					hasStaff3 = true;
				...
				if (spell == 23 && c.getItems().getInventoryCount(24611) > 0) {
					//snare
					hasStaff1 = true;
					hasStaff2 = true;
					hasStaff3 = true;
				...
				if (spell == 24 && c.getItems().getInventoryCount(24613) > 0) {
					//entangle
					hasStaff1 = true;
					hasStaff2 = true;
					hasStaff3 = true;
			...
			if (c.playerAttackingIndex > 0) {
				if (getPlayers().get(c.playerAttackingIndex) != null) {
					for (int r = 0; r < CombatSpellData.REDUCE_SPELLS.length; r++) { //Reducing/weaken spells
						if (CombatSpellData.REDUCE_SPELLS[r] == CombatSpellData.MAGIC_SPELLS[spell][0]) {
							c.reduceSpellId = r;
							Player.canUseReducingSpell[c.reduceSpellId] = (System.currentTimeMillis() - getPlayers().get(c.playerAttackingIndex).reduceSpellDelay[c.reduceSpellId]) > CombatSpellData.REDUCE_SPELL_TIME[c.reduceSpellId];
							break;
						}
					}
			...
			if ((c.playerEquipment[Player.playerWeapon] == 11791 || c.playerEquipment[Player.playerWeapon] == 12904) && RandomUtils.nextInt(0, 100) < 13 || c.playerEquipment[Player.playerWeapon] == 24144) {
				runesNecessary = false;
			}

			if (c.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33090) && Misc.random(1,20) <= 3) {
				deleteRunes = false;
			}

			if (deleteRunes) {
				if (!Boundary.isIn(c, Boundary.FOUNTAIN_OF_RUNE_BOUNDARY)) {
					if (Configuration.RUNES_REQUIRED && runesNecessary && !blighted) {
	...
	private static void HandleBlighted(Player player, int spellID) {
		switch (spellID) {
			case 47:
				player.getItems().deleteItem2(24607, 1);
				break;
			case 55:
				player.getItems().deleteItem2(24621, 1);
				break;
			case 22:
				player.getItems().deleteItem2(24609, 1);
				break;
			case 23:
				player.getItems().deleteItem2(24611, 1);
				break;
			case 24:
				player.getItems().deleteItem2(24613, 1);
				break;
		}
	}
}