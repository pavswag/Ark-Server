...
public class LunarSpells extends MagicRequirements {
	...
	public static void CastingLunarOnPlayer(final Player c, int castingSpellId) {
        final Player castOnPlayer = getPlayers().get(c.playerAttackingIndex);
		...
		switch(castingSpellId) {
			...
			break;
			case 30298:
				if (!castOnPlayer.acceptAid) {//this is for casting veng on others
					c.sendMessage("This player is currently not accepting aid.");
					return;
				}
				...
				c.getPA().sendConfig(2450, 1);
				c.getPA().sendConfig(2451, 1);
				CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
					@Override
					public void execute(CycleEventContainer container) {
						c.getPA().sendConfig(2451, 0);
						container.stop();
					}
				}, 50);
			...
			case 30290:
				...
				c.appendDamage(c, (int) hpPercent, hpPercent >= 1.0 ? HitMask.HIT : HitMask.MISS);
