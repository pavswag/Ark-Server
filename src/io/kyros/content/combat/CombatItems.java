...
public class CombatItems {
	...
	public double getInquisitorBonus() {
		WeaponInterface weaponInterface = c.getCombatConfigs().getWeaponData().getWeaponInterface();
		if (c.attacking.getCombatType() == CombatType.MELEE && (weaponInterface == WeaponInterface.SWORD_CRUSH || weaponInterface == WeaponInterface.HALBERD || weaponInterface == WeaponInterface.MAUL || weaponInterface == WeaponInterface.MACE)) {
			double bonus = 100;
	...
	public boolean elyProc() {
		...
		if (c.playerEquipment[Player.playerShield] == 27253 && Misc.trueRand(10) <= 6) {
			c.startGraphic(new Graphic(2109));
			return true;
		}
		if (c.playerEquipment[Player.playerShield] == 27251 && Misc.trueRand(10) <= 6) {
			c.startGraphic(new Graphic(2102));
			return true;
		}
		if (c.playerEquipment[Player.playerShield] == 25985 && Misc.trueRand(10) <= 6) {
			c.startGraphic(new Graphic(2100));
			return true;
		}
		return false;
	}
	...
	public boolean usingBofa() {
		return (c.playerEquipment[Player.playerWeapon] >= 25865 && c.playerEquipment[Player.playerWeapon] <= 25869) || (c.playerEquipment[Player.playerWeapon] >= 25884 && c.playerEquipment[Player.playerWeapon] <= 25896) || c.playerEquipment[Player.playerWeapon] == 27187;
	}

	public boolean usingCrystalGear() {
		return c.playerEquipment[Player.playerHat] == 23842 && c.playerEquipment[Player.playerChest] == 23845 && c.playerEquipment[Player.playerLegs] == 23848;
	}
	...
	public boolean usingBlowPipe() {
		return c.playerEquipment[Player.playerWeapon] == Items.TOXIC_BLOWPIPE || c.playerEquipment[Player.playerWeapon] == 28688;
	}
	...
	public boolean usingDbow() {
		return c.playerEquipment[Player.playerWeapon] == 11235 || c.playerEquipment[Player.playerWeapon] == 12765 || c.playerEquipment[Player.playerWeapon] == 12766
				|| c.playerEquipment[Player.playerWeapon] == 12767 || c.playerEquipment[Player.playerWeapon] == 12768 || c.playerEquipment[Player.playerWeapon] == 29599 || c.playerEquipment[Player.playerWeapon] == 33434;
	}

	public boolean properBolts() {
		int i = c.playerEquipment[Player.playerArrows];
		if (c.playerEquipment[Player.playerWeapon] == 8880 && i == 8882) {
			return true;
		}
	...
	public boolean usingJavelins(int i) {
		return (i >= 825 && i <= 830) || i == 19484 || i == 21318 || i == 22636;
	}
	...
	public boolean usingAccumulator() {
		return c.getItems().isWearingItem(Items.AVAS_ACCUMULATOR)
				...
				|| c.getItems().isWearingItem(Items.COMPLETIONIST_CAPE)
				|| c.getItems().isWearingItem(23859);
	}
	...
	public void checkBlowpipe() {
		if (c.getItems().isWearingItem(Items.TOXIC_BLOWPIPE, Player.playerWeapon) || c.getItems().isWearingItem(28688, Player.playerWeapon)) {
			...
			if (c.getToxicBlowpipeAmmoAmount() <= 0 && c.getToxicBlowpipeCharge() <= 0) {
				...
				c.getItems().addItemUnderAnyCircumstance(c.getItems().isWearingItem(28688, Player.playerWeapon) ? 28687 : 12924, 1);
