...
public class RechargeItems {
	...
	public boolean hasItem(int itemId) {
        return player.getItems().playerHasItem(itemId) || player.getItems().isWearingItem(itemId);
    }
	...
	public void replenishRun(int amount) {
		...
		if (total > 100) {
			...
			player.setRunEnergy(player.getRunEnergy() + amount, true);
		}
	}
	...
	public enum ItemsWithRecharge {
		...
		ItemsWithRecharge(int itemId, int charges, long time) {
			this.itemId = itemId;
	 ...
	 */
	private final Map<Integer, Integer> itemValues = new HashMap<>();
	private final Map<Integer, String> itemLastUsed = new HashMap<>();
	...
	public String getItemLastUsed(int itemId) {
		return itemLastUsed.get(itemId);
	}
