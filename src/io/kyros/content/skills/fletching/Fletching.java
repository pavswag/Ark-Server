...
public class Fletching {
	...
	public static final List<Integer> Carp_ids =
			Arrays.asList(
					24872, 24874, 24876, 24878);
	public static int getFletchEquipmentCount(Player player) {
		int count = 1;
		for (int i : Carp_ids) {
		...
		selectedGroup.ifPresent(group -> {
			for (FletchableLog fletchable : group.getFletchables()) {
				...
				if (index != -1) {
					...
					if (amount == 1 && player.amDonated >= 50) {
						amount = 100;
					}
	...
	public void fletchLog(FletchableLog fletchable, int amount) {
		...
		Server.getEventHandler().submit(new FletchLogEvent(player, (player.amDonated >= 100 ? 1 : 3), fletchable, amount));
	}
	...
	public void fletchUnstrung(int bowId) {
		...
		bow.ifPresent(b -> {
			...
			Server.getEventHandler().submit(new StringBowEvent(b, player, (player.amDonated >= 100 ? 1 : 3)));
		});
	}
	...
	public void fletchUnstrungCross(int crossbowId) {
		...
		crossbow.ifPresent(b -> {
			...
			Server.getEventHandler().submit(new StringCrossbowEvent(b, player, (player.amDonated >= 100 ? 1 : 3)));
		});
	}
