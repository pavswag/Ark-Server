package io.kyros.content.skills.agility.impl.rooftop;
...
import io.kyros.content.skills.agility.MarkOfGrace;
...
public class RooftopVarrock {
	...
	public static boolean execute(final Player c, final int objectId) {
		...
		case CLOTHES_LINE:
			...
			if (c.distance(new Position(3214, 3414)) < 2.5) {
			...
			CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					...
					if (c.absX == 3190 && c.absY == 3414) {
						...
						c.setForceMovement(3190, 3408, 0, 150, "SOUTH", 756);
					} else if (c.absX == 3190 && c.absY == 3408) {
		...
		case LEAP_2ND_GAP:
			if (c.getAgilityHandler().agilityProgress[3] == true) {
				...
				c.appendDamage(null, 1, HitMask.HIT);
		...
		case JUMP_OFF_EDGE:
			...
			AgilityHandler.delayEmote(c, "JUMP", 3221, 3414, 0, 2);
