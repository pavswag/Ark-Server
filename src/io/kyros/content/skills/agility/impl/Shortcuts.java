package io.kyros.content.skills.agility.impl;
	...
	public boolean agilityShortcuts(final Player c, final int objectId) {
		...
		case 4469:
			...
			case 16510:
				...
				return true;
			case 16509:
				if(c.playerLevel[Player.playerAgility] < 70) {
				...
				return true;
		case 23274:
			if (c.playerLevel[Player.playerAgility] < 20) {
		...
		case 29082:
			...
			npc.forceChat("HAH! Off you go " + c.getDisplayName() + "!");
		...
		case 678: // Cave to corp
			if (c.getPosition().inWild()) {
				...
				if (Server.isDebug()) {
					c.sendMessage("Ignoring key because debug mode.");
				} else if (!c.getItems().playerHasItem(23951)) {
					c.sendMessage("You need a crystalline key to get in here.");
					return true;
				}
		...
		case 679: // Cave out from corp
			WildWarning.sendWildWarning(c, plr -> AgilityHandler.delayFade(c, "CRAWL", 3206, 3681, 0, "You crawl into the cave.",
...
//			return true;
		...
		case SLAYER_TOWER_CHAIN_UP:
			...
			if (z == 0 && y <= 3552) {
				if (c.playerLevel[Player.playerAgility] < 60) {
		...
		case 17050:
		case 17049:
			if (c.playerLevel[Player.playerAgility] < 11 || c.playerLevel[Player.playerStrength] < 37
					|| c.playerLevel[Player.playerRanged] < 19) {
				c.sendMessage(
