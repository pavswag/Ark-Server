...
public class Farming {
	...
	public void doConfig() {
		...
		switch (closest) {
			case DZ_HERB:
				config = (config(FarmingPatches.DZ_HERB) << 24);
				break;
			case DZ_HERB_NEW:
				config = (config(FarmingPatches.DZ_HERB_NEW) << 24);
				break;
			case FALADOR_HERB:
				...
				break;
			case WILDY_HERB:
				config = (config(FarmingPatches.WILDY_HERB) << 24)
						+ (config(FarmingPatches.WILDY_FLOWER) << 16)
						+ (config(FarmingPatches.WILDY_ALLOTMENT_SOUTH) << 8)
						+ (config(FarmingPatches.WILDY_ALLOTMENT_NORTH));
				break;
			case CATHERBY_HERB:
				/**
				 * //		529(<<0) = Fruit Tree
				...
				config = (config(FarmingPatches.CATHERBY_FRUIT_TREE))
						+ (config(FarmingPatches.CATHERBY_HERB) << 24)
...
//				break;
	...
	private boolean isPatchException(int x, int y, FarmingPatches patch) {
		if(x == 3054 && y == 3307 && patch != FarmingPatches.FALADOR_FLOWER)
			return true;
		if(x == 3139 && y == 3670 && patch != FarmingPatches.WILDY_FLOWER)
			return true;
					...
					if (plant.seed == seed) {
						if (player.getPA().getLevelForXP(player.playerXP[Skill.FARMING.getId()]) >= plant.level) {
							...
							if (player.getItems().playerHasItem(patch.planter)) {
								...
								if(player.getCurrentPet().findPetPerk("uncommon_seedy").isHit()) {
									player.sendMessage("Your Seedy pet perk has preserved your seed!");
								} else {
								...
								if ((player.amDonated >= 100 && Boundary.isIn(player, Boundary.DONATOR_ZONE)) ||
										(player.amDonated >= 100 && Boundary.isIn(player, Boundary.DONATOR_ZONE_NEW))) {
