...
public class Slayer {
	 ...
	 */
	private static final int BOSS_TASK_EXPERIENCE = 1500;
		...
		SlayerMaster.get(masterId).ifPresent(m -> {
			if (player.slayerParty && !player.slayerPartner.isEmpty()) {
				...
				if (p != null && p.calculateCombatLevel() < m.getLevel() && !override) {
					player.start(new DialogueBuilder(player).statement(p.getDisplayName()+ " needs a combat level of " + m.getLevel() + " to receive tasks from me.", "Please come back when they have this combat level."));
					return;
				} else if (p != null && p.getSlayer().getTaskAmount() > 0) {
					player.start(new DialogueBuilder(player).statement(p.getDisplayName() + " already has a task they need to cancel this task first."));
					return;
				}
			}
			...
			if (exclusiveLength == 0) {
				...
				String[] msg = { "You need to unblock some tasks!", "YOU HAVE BLOCKED TO MANY TASKS!" };
...
//			}
			...
			if (player.slayerParty && !player.slayerPartner.equalsIgnoreCase("")) {
				for (Player p : Server.getPlayers().toPlayerArray()) {
...
//		displayInterface();
			...
			if (isTaskNpc(npc)) {
				...
				master.ifPresent(m -> {
					...
					if (taskAmount == 0) {
						...
						if (player.getSlayer().getMaster() == 7663) {
							int rng = Misc.random(50,5000);
							player.getItems().addItemUnderAnyCircumstance(13307, rng);
						}
						int multiplier = 1;
						...
						if (Hespori.activeEnhancedNoxiferSeed) {
							multiplier += 2;
						} else if (Hespori.activeNoxiferSeed) {
							multiplier += 1;
						}
						if (player.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33078)) {
							multiplier += 1;
						}
						if (player.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33075)
								&& player.wildLevel > 0) {
							multiplier += 1;
						}

						if (Misc.random(0, 100) > 95 && !(player.getSlayer().getMaster() == 401 && !(player.getSlayer().getMaster() == 402))
								&& player.TaskExtended && NewInterface.getBlockedTasks(player) <= 0) {
							player.sendMessage("@red@You received a Overcharged Cell for your efforts!");
						}
						...
						if (Misc.random(0, 100) > 85) {
							if (!player.skillingMinigame) {
								player.getItems().addItemUnderAnyCircumstance(11640, 1);
							}
						}
						...
						if (Boundary.isIn(player, DonoSlayerInstances.boundary)) {
							player.getPA().startTeleport(2416, 3826, 0, "foundry", false);
							player.sendMessage("Your task has ended and you have been removed from the Donation Slayer Instance!");
						}
						player.TaskExtended = false;

						if (Misc.random(0, 100) > 95) {
							player.getSlayer().getUnlocks().add(SlayerRewardsInterfaceData.Unlock.SUPER_SLAYER_HELM.getUnlock());
							player.sendMessage("@red@Congratulation's you've unlocked Super Slayer Helm crafting!");
							player.sendMessage("@red@Congratulation's you've unlocked Super Slayer Helm crafting!");
						}
						...
						if (player.getSlayer().getMaster() != 401 || player.getSlayer().getMaster() != 402) {
							if (player.amDonated >= 20 && player.amDonated < 50) {
								...
								points += 30;
								player.sendMessage("You have received an additonal@red@ 30@bla@ points for your donator rank.");
							}
						}
	...
	public void handleSuperiorSpawn() {
		task.ifPresent(task -> {
			...
			if (chance == 0 || Server.isDebug()) {
				...
				//superiorSpawned = true;
				//player.sendMessage("@red@A superior foe has appeared...");
				player.sendSpamMessage("A superior foe has appeared...");
			}
		});
	}
	...
	public void removeTask() {
		...
		for (int index = 0; index < removed.length; index++) {
			if (index > 2 && !player.getRights().isOrInherits(Right.Donator)) {
	...
	public boolean unlock(SlayerUnlock unlock, int cost) {
		if (unlocks.contains(unlock) && unlock != SlayerRewardsInterfaceData.Unlock.IMBUE_SLAYER_HELMET.getUnlock()) {
			return false;
		} else if (points >= cost) {
			switch (unlock) {
				case IMBUE_HELMET:
					...
					if (!player.getItems().playerHasItem(11864) &&
							...
							(!player.getItems().playerHasItem(25898) && player.getSlayer().getUnlocks().contains(NewInterface.Unlock.SUPER_SLAYER_HELM.getUnlock()) ||
									!player.getItems().playerHasItem(25904) && player.getSlayer().getUnlocks().contains(NewInterface.Unlock.SUPER_SLAYER_HELM.getUnlock()))*/) {
					...
					if (player.getItems().playerHasItem(25898) && player.getSlayer().getUnlocks().contains(NewInterface.Unlock.SUPER_SLAYER_HELM.getUnlock())) {
						slayerHelmet = 25898;
						slayerHelmetI = 25900;
					}
					if (player.getItems().playerHasItem(25904) && player.getSlayer().getUnlocks().contains(NewInterface.Unlock.SUPER_SLAYER_HELM.getUnlock())) {
						slayerHelmet = 25904;
						slayerHelmetI = 25906;
					}
					points -= cost;
...
//			updatePoints();
	...
	public void displayInterface() {
		if (player.getSlayer().getTask().isPresent()) {
			player.getPA().sendString(35425, "task: " + player.getSlayer().getTask().get().getPrimaryName().replace(" ", "_") + " " + player.getSlayer().getTaskAmount() + " " + player.getSlayer().getConsecutiveTasks() + " " + player.getSlayer().getPoints());
