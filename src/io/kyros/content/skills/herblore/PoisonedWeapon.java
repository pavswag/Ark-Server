...
public enum PoisonedWeapon {
	...
	BONE_DAGGER_P_PLUS_PLUS(PoisonLevel.PLUSPLUS, 8872, 8878),
	BRONZE_ARROW_P(PoisonLevel.NORMAL, 882, 883),
	BRONZE_ARROW_P_PLUS(
PoisonLevel.PLUS,
882,
...
6593), WHITE_DAGGER_P_PLUS(
	...
	private final PoisonLevel level;
	 ...
	 *
	 * @param poisonedWeapon The item id of the poisoned weapon.
	...
	public static boolean poisonWeapon(Player player, int item1, int item2) {
		Optional<Integer> poison = getPoison(item1, item2);
		if (!poison.isPresent()) {
			return false;
		}
		int weapon;
		...
		Optional<Integer> poisonedWeapon = getResult(poison.get(), weapon);
		if (poisonedWeapon.isPresent()) {
			int amount = 1;
			if (ItemDef.forId(weapon).isStackable()) {
			...
			player.sendMessage("You poison the " + ItemDef.forId(weapon).getName() + ".");
		}
		return false;
	}
