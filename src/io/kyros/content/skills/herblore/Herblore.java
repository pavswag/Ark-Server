...
import static io.kyros.content.skills.firemake.Firemaking.pyromancerOutfit;
				...
				public void execute(CycleEventContainer container) {
					...
					if (p.name().equalsIgnoreCase("prayer")) {
						for (TaskMasterKills taskMasterKills : player.getTaskMaster().taskMasterKillsList) {
							if (taskMasterKills.getDesc().equalsIgnoreCase("Craft @whi@Prayer pot's")) {
					...
					case SUPER_COMBAT_4:
						...
						for (TaskMasterKills taskMasterKills : player.getTaskMaster().taskMasterKillsList) {
							if (taskMasterKills.getDesc().equalsIgnoreCase("Craft @whi@Combat Pot's")) {
								taskMasterKills.incrementAmountKilled(1);
								player.getTaskMaster().trackActivity(player, taskMasterKills);
								break;
							}
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (player.getItems().playerHasItem(227) && player.getItems().playerHasItem(unf.getHerb().getId())) {
					if(player.getCurrentPet().findPetPerk("common_Mixology").isHit()) {
						player.sendMessage("<icon=9999> Your pet has saved you some potion materials!");
					} else {
