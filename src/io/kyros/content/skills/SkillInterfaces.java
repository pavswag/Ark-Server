...
import io.kyros.content.skills.herblore.PotionData.FinishedPotions;
...
public class SkillInterfaces {
	...
	private void writeInterfaceItem(int[] id) {
		...
		c.outStream.writeInt(8847); // 8847
		c.outStream.writeShort(id.length);
	...
	public void attackComplex(int screen) {
		if (screen == 1) {
			...
			menuLine(level, type + " Halberd", 9431, 2);
			optionTab("Attack", type, "Bronze", "Iron", "Steel", "Black", "White", "Mithril", "Adamant", "Rune", "Dragon", "Barrows", "Special", "Milestones", "");
		} else if (screen == 9) {
			clearMenu();
			String level = "60";
			...
			menuLine("75", "Scythe Of Vitur", 22325, 13);
			menuLine("75", "Ghrazi Rapier", 22324, 14);
			optionTab("Attack", "Special", "Bronze", "Iron", "Steel", "Black", "White", "Mithril", "Adamant", "Rune", "Dragon", "Barrows", "Special", "Milestones", "");
		} else if (screen == 12) {
			clearMenu();
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[Player.playerAttack][0], 0);
	...
	public void strengthComplex(int screen) {
		if (screen == 1) {
			...
			clearMenu();
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[Player.playerStrength][0], 0);
	...
	public void defenceComplex(int screen) {
		if (screen == 1) {
			...
			menuLine("70", "Dinh's bulwark", 21015, 28);
			menuLine("75", "Justiciar Faceguard", 22326, 29);
			menuLine("75", "Justiciar Chestguards", 22327, 30);
			menuLine("75", "Justiciar Legguards", 22328, 31);
			optionTab("Defence", "Equipment", "Bronze", "Iron", "Steel", "Black", "White", "Mithril", "Adamant", "Rune", "Dragon", "Barrows", "Magic", "Equipment", "Milestones");
		}

		else if (screen == 13) {
			clearMenu();
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[Player.playerDefence][0], 0);// Skill
																							// mastery
	...
	public void rangedComplex(int screen) {
		if (screen == 1) {
			...
			menuLine("75", "Toxic Blowpipe", 12926, 10);
			menuLine("75", "Twisted Bow", 20997, 11);
			...
			menuLine("70", "Armadyl Helmet(With 70 Defence)", 11826, 33);
			menuLine("70", "Armadyl Chestplate(With 70 Defence)", 11828, 34);
			menuLine("70", "Armadyl Chainskirt(With 70 Defence)", 11830, 35);
			...
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[Player.playerRanged][0], 1);
	...
	public void prayerComplex(int screen) {
		if (screen == 1) {
			...
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[Player.playerPrayer][0], 1);
	...
	public void magicComplex(int screen) {
		if (screen == 1) {
			...
			menuLine("75", "Ancestral Hat(With 65 Defence)", 21018, 29);
			menuLine("75", "Ancestral Robe Top(With 65 Defence)", 21021, 30);
			menuLine("75", "Ancestral Robe Bottoms(With 65 Defence)", 21024, 31);
			...
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[Player.playerMagic][0], 1);
	...
	public void hitpointsComplex(int screen) {
		if (screen == 1) {
			...
			clearMenu();
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[Player.playerHitpoints][0], 0);
	...
	public void agilityComplex(int screen) {
		if (screen == 1) {
			...
			menuLine("70", "Pollnivneach Rooftop Agility Course", 11743, 5);// charos
			menuLine("80", "Rellekka Rooftop Agility Course", 11744, 6);// charos
			menuLine("90", "Ardougne Rooftop Agility Course", 8011, 7);// charos
			...
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[Player.playerAgility][0], 1);
	...
	public void herbloreComplex(int screen) {
		if (screen == 1) {
			...
			FinishedPotions[] ignore = {
					FinishedPotions.ANTI_VENOM_4, FinishedPotions.ANTI_VENOM_2, FinishedPotions.ANTI_VENOM_1,
					FinishedPotions.SANFEW_4,
					FinishedPotions.DIVINE_MAGIC_POTION_3,
					FinishedPotions.DIVINE_RANGING_POTION_3,
					FinishedPotions.DIVINE_SUPER_COMBAT_3,
					FinishedPotions.SUPER_COMBAT_3
			};
			...
			int lineCounter = 0;
			for (FinishedPotions potion : potions) {
				StringJoiner builder = new StringJoiner("");
				builder.add(potion.getResult().getDef().getName());
				builder.add("\\n");

				StringJoiner ing = new StringJoiner(", ");

				if (potion == FinishedPotions.GUTHIX_BALANCE) {
					ing.add("Rannar pot(unf)");
					ing.add("Spider eggs");
					ing.add("Garlic");
					ing.add("Silver dust");
				} else {

					ing.add(potion.getPrimary().getDef().getName());
					if (potion == FinishedPotions.SUPER_COMBAT_4) {
						ing.add("Super set");
					} else {
...
//			menuLine("90", "Super Combat \\n Torstol potion (unf), 1 x super attack, strength\\nand defence potion", 12695, 26);
			...
			clearMenu();
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[Player.playerHerblore][0], 0);
	...
	public void thievingComplex(int screen) {
		if (screen == 1) {
			...
			clearMenu();
			menuLine("99", "Skill mastery", c.getItems().skillcapes[Player.playerThieving][0], 0);
	...
	public void fletchingComplex(int screen) {
		if (screen == 1) {
			...
			clearMenu();
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[Player.playerFletching][0], 0);
			optionTab("Fletching", "Darts", "Arrows", "Bows", "Darts", "Javelins", "Milestones", "", "", "", "", "", "", "", "");
		}
	}
	...
	public void slayerComplex(int screen) {
		if (screen == 1) {
			...
			clearMenu();
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[Player.playerSlayer][0], 0);
	...
	public void fishingComplex(int screen) {
		if (screen == 1) {
			...
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[Player.playerFishing][0], 1);
	...
	public void cookingComplex(int screen) {
		if (screen == 1) {
			...
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[Player.playerCooking][0], 1);
	...
	public void firemakingComplex(int screen) {
		if (screen == 1) {
			...
			clearMenu();
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[Player.playerFiremaking][0], 0);
	...
	public void woodcuttingComplex(int screen) {
		if (screen == 1) {
			...
			clearMenu();
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[Player.playerWoodcutting][0], 0);
	...
	public void farmingComplex(int screen) {
		if (screen == 1) {
			...
			clearMenu();
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[Player.playerFarming][0], 0);
