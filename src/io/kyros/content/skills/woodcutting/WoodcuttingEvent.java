	...
	public void execute() {
		...
		if (!tree.equals(Tree.NORMAL)) {
			if (Misc.random(chopChance) == 1 || chops >= tree.getChopsRequired()) {
				...
				attachment.getPA().addSkillXPMultiplied((int) osrsExperience, Skill.WOODCUTTING.getId(), true);
				Achievements.increase(attachment, AchievementType.WOODCUT, 1);
				if ((attachment.getItems().isWearingItem(25066) || (attachment.getItems().isWearingItem(13241) || attachment.getItems().playerHasItem(13241)) || attachment.getItems().playerHasItem(25066) || attachment.getItems().playerHasItem(25110)) && random == 2) {
				...
				if (attachment.getCurrentPet().hasPerk("uncommon_transmute")) {
					if (attachment.getCurrentPet().findPetPerk("uncommon_transmute").isHit()) {
						switch (tree) {
                            case OAK -> {
                                attachment.getItems().addItemUnderAnyCircumstance(Tree.WILLOW.getWood(), 1);
                                break;
                            }
							case WILLOW -> {
								attachment.getItems().addItemUnderAnyCircumstance(Tree.MAPLE.getWood(), 1);
								break;
							}
							case YEW -> {
								attachment.getItems().addItemUnderAnyCircumstance(Tree.MAGIC.getWood(), 1);
								break;
							}
							case MAPLE -> {
								attachment.getItems().addItemUnderAnyCircumstance(Tree.YEW.getWood(), 1);
								break;
							}
                        }
					}
