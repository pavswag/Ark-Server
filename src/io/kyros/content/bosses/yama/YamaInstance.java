...
public class YamaInstance extends InstancedArea {

    public static Boundary YAMA_ZONE = new Boundary(3355, 9733, 3390, 9764);
    ...
    public YamaInstance() {super(CONFIGURATION, YAMA_ZONE);
    }

    public void enter(Player player) {
        try {
            player.moveTo(new Position(3379, 9757, getHeight()));
            add(player);
            NPC npc = new YamaNPC(10936, new Position(3373, 9745, getHeight()));
            add(npc);
    ...
    public static void JoinInstance(Player player) {
        player.start(new DialogueBuilder(player).option("Would you like to start an instance or enter one?", new DialogueOption("Start an instance", p -> {
            YamaInstance yam = new YamaInstance();
            yam.enter(p);
    ...
    private static boolean canJoinInstance(YamaInstance instance) {
        ...
        if (npc.getInstance() != null) {
            for (Player player : npc.getInstance().getPlayers()) {
                if (player != null && !player.isIdle) {
                    ...
                    Server.getDropManager().create(player, npc, new Location3D(player.getX(), player.getY(), player.getHeight()), amountOfDrops, 10936);
                    player.getNpcDeathTracker().add(npc.getName(), npc.getDefinition().getCombatLevel(), 1);
                    Achievements.increase(player, AchievementType.SLAY_YAMA, 1);
                }
            }
        }
    }
