...
public class MinoInstance extends LegacySoloPlayerInstance {
    ...
    public MinoInstance(Player player) {
        super(CONFIGURATION, player, Boundary.NEW_INSTANCE_AREA);
    }

    public void enter(Player player, MinoInstance instance) {
        ...
        try {
            player.MinotaurInstanceKills = Misc.random(100,350);

            player.sendErrorMessage("You've instanced Minotaur for " + player.MinotaurInstanceKills + " Kills!");
            int minX = 1944;
            int minY = 3738;
            int maxX = 1962;
            int maxY = 3755;
            ...
            int maxNpcsWidth = availableWidth / (1 + NpcDef.forId(12813).getSize());
            int maxNpcsHeight = availableHeight / (1 + NpcDef.forId(12813).getSize());
            int maxNpcs = maxNpcsWidth * maxNpcsHeight;

            for (int i = 0; i < maxNpcs; i++) {
                int x = minX + (i % maxNpcsWidth) * (1 + NpcDef.forId(12813).getSize());
                int y = minY + (i / maxNpcsWidth) * (1 + NpcDef.forId(12813).getSize());

                NPC slayer_npc = NPCSpawning.spawnNpc(player, 12813, x, y, instance.getHeight(),0,0,false,false);
                ...
                slayer_npc.getHealth().setMaximumHealth(25000);
                slayer_npc.getHealth().setCurrentHealth(25000);
