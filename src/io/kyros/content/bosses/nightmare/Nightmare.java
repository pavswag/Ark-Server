...
import io.kyros.content.bosses.nightmare.attack.SleepWalkers;
import io.kyros.content.bosses.nightmare.attack.StandardMage;
import io.kyros.content.bosses.nightmare.attack.StandardMelee;
import io.kyros.content.bosses.nightmare.attack.StandardRanged;
import io.kyros.content.bosses.nightmare.phase.Phase1;
import io.kyros.content.bosses.nightmare.phase.Phase2;
import io.kyros.content.bosses.nightmare.phase.Phase3;
import io.kyros.content.bosses.nightmare.totem.TotemSpawn;
import io.kyros.content.bosses.nightmare.totem.Totems;
...
import static io.kyros.content.bosses.nightmare.NightmareConstants.*;
...
public class Nightmare extends NPC {
    ...
    public void kill() {
        getInstance().getNpcs().stream().filter(npc -> !npc.equals(this) && !TotemSpawn.isTotem(npc))
                .forEach(npc -> npc.appendDamage(npc, npc.getHealth().getCurrentHealth(), HitMask.HIT));
        deathTimer = 16;
    ...
    private boolean processDeath() {
        ...
        if (deathTimer == 0) {
            ...
            players.forEach(plr -> {
                for (TaskMasterKills taskMasterKills : plr.getTaskMaster().taskMasterKillsList) {
                    if (taskMasterKills.getDesc().contains("Nightmare")) {
                        taskMasterKills.incrementAmountKilled(1);
                        plr.getTaskMaster().trackActivity(plr, taskMasterKills);
                    }
                }
                ...
                plr.moveTo(new Position(3808, 9755, 1));
                plr.sendErrorMessage("You were moved to start a new instance!");
            });
            resetForSpawn();
            return false;
