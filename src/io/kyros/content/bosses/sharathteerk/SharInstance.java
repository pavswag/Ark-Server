...
public class SharInstance extends LegacySoloPlayerInstance {
    ...
    public SharInstance(Player player) {
        super(CONFIGURATION, player, Boundary.NEW_INSTANCE_AREA);
    }

    public void enter(Player player, SharInstance instance) {
        ...
        try {
            player.SharInstanceKills = Misc.random(100,350);

            player.sendErrorMessage("You've instanced Sharathteerk for " + player.SharInstanceKills + " Kills!");
            int minX = 1944;
            int minY = 3738;
            int maxX = 1962;
            int maxY = 3755;
            ...
            int maxNpcsWidth = availableWidth / (1 + NpcDef.forId(12617).getSize());
            int maxNpcsHeight = availableHeight / (1 + NpcDef.forId(12617).getSize());
            int maxNpcs = maxNpcsWidth * maxNpcsHeight;

            for (int i = 0; i < maxNpcs; i++) {
                int x = minX + (i % maxNpcsWidth) * (1 + NpcDef.forId(12617).getSize());
                int y = minY + (i / maxNpcsWidth) * (1 + NpcDef.forId(12617).getSize());

                NPC slayer_npc = NPCSpawning.spawnNpc(player, 12617, x, y, instance.getHeight(),0,0,false,false);
