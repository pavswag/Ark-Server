...
/*		if (player.getItems().getItemCount(19835, false) > 0) {
		...
		if (player.getItems().playerHasItem(2677) && player.getItems().playerHasItem(2801) && player.getItems().playerHasItem(2722)) {
			int smallest, temp;
			int easy = player.getItems().getInventoryCount(2677);
			int medium = player.getItems().getInventoryCount(2801);
			int hard = player.getItems().getInventoryCount(2722);
			temp = Math.min(easy, medium);
			smallest= Math.min(hard, temp);
			player.getItems().deleteItem(2677, smallest);
			player.getItems().deleteItem(2801, smallest);
			player.getItems().deleteItem(2722, smallest);
			player.getItems().addItem(19835, smallest);
