package io.kyros.content.trails;
...
public class TreasureTrails {
	...
	public List<GameItem> generateRewardList(RewardLevel rewardLevel, int rolls) {
		player.getPA().sendJingle(Jingles.CLUE_REWARD);
...
//				announceRare(player, new GameItem(item, 1), rewardLevel);
	...
	public void addRewards(RewardLevel rewardLevel) {
		...
		rewards.forEach(it -> player.getItems().addItemToBankOrDrop(it.getId(), it.getAmount()));
		displayRewards(rewards);
	}

	public void displayRewards(List<GameItem> rewards) {
		player.outStream.createFrameVarSizeWord(53);
		player.outStream.writeInt(6963);
		player.outStream.writeShort(rewards.size());
	...
	private static void openCasket(Player player, RewardLevel rewardLevel) {
		...
		int amt = player.getItems().getInventoryCount(rewardLevel.getCasketId());

		for (int i = 0; i < amt; i++) {
			player.getItems().deleteItem(rewardLevel.getCasketId(), 1);
			player.getTrails().addRewards(rewardLevel);

			if (player.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33114) && Misc.isLucky(5)) {
//				player.sendMessage("@red@You manage to get another clue casket!!");
