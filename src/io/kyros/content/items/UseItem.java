...
import io.kyros.content.PlatinumTokens;
...
import io.kyros.content.item.lootable.impl.CrystalChest;
import io.kyros.content.item.lootable.impl.FoeMysteryBox;
import io.kyros.content.items.item_combinations.Godswords;
...
import io.kyros.content.skills.crafting.GemCutting;
...
import io.kyros.content.skills.crafting.SpinMaterial;
import io.kyros.content.skills.firemake.Burner;
import io.kyros.content.skills.firemake.Firemaking;
import io.kyros.content.skills.firemake.LogData;
import io.kyros.content.skills.fishing.ArielFishing;
import io.kyros.content.skills.herblore.Crushable;
...
import io.kyros.content.trails.MasterClue;
...
import io.kyros.util.logging.player.ItemOnItemLog;
...
public class UseItem {
		...
		switch (objectID) {
			...
			case 2031:
				...
				switch (itemId) {
					...
					case 763:
					case 765:
						if (c.getItems().playerHasItem(763)
								&& c.getItems().playerHasItem(765)
								&& c.getItems().playerHasItem(2347)
								&& c.getItems().getInventoryCount(2356) >= 250) {

							c.startAnimation(898);

							c.getItems().deleteItem2(763, 1);
							c.getItems().deleteItem2(765, 1);
							c.getItems().deleteItem2(2347, 1);
							c.getItems().deleteItem2(2356, 250);
							c.getItems().addItemUnderAnyCircumstance(28807, 1);
							c.sendErrorMessage("You have created the Shield Of Arrav, speak to Ted near the tournament portal!");
						}
						break;
	...
	public static void ItemonItem(final Player c, final int itemUsed, final int useWith, final int itemUsedSlot, final int usedWithSlot) {
		...
		if (itemUsed == 20366 && useWith == 29799 || itemUsed == 29799 && useWith == 20366) {

			if (c.getItems().playerHasItem(20366) && c.getItems().playerHasItem(29799) &&
			c.foundryPoints >= 10_000_000_000L){

				c.getItems().deleteItem2(20366, 1);
				c.getItems().deleteItem2(29799, 1);
				c.foundryPoints -= 10_000_000_000L;
				c.getItems().addItemUnderAnyCircumstance(29801, 1);
			} else {
				c.sendErrorMessage("You need Torture Ammy (or) & Araxyte Fang + 10B Nomad to make this amulet.");
			}
		}

		if (itemUsed == 29790 && useWith == 29792 || itemUsed == 29792 && useWith == 29790) {

			if (c.getItems().playerHasItem(29790) && c.getItems().playerHasItem(29794) && c.getItems().playerHasItem(29794) &&
			c.foundryPoints >= 10_000_000_000L){

				c.getItems().deleteItem2(29790, 1);
				c.getItems().deleteItem2(29792, 1);
				c.getItems().deleteItem2(29794, 1);
				c.foundryPoints -= 10_000_000_000L;
				c.getItems().addItemUnderAnyCircumstance(29796, 1);
			} else {
				c.sendErrorMessage("You need Nox Point, Nox Blade, Nox Pommel + 10B Nomad to make the halberd");
			}
		}

		if (itemUsed == 28319 && useWith == 28323) {

			if (c.getItems().playerHasItem(28319) &&
			c.getItems().playerHasItem(28323) &&
			c.getItems().playerHasItem(28321) &&
			c.getItems().playerHasItem(28325) &&
			c.foundryPoints >= 10_000_000_000L) {

				c.getItems().deleteItem2(28319, 1);
				c.getItems().deleteItem2(28323, 1);
				c.getItems().deleteItem2(28321, 1);
				c.getItems().deleteItem2(28325, 1);
				c.foundryPoints -= 10_000_000_000L;
				c.getItems().addItemUnderAnyCircumstance(28338, 1);
			} else {
				c.sendErrorMessage("You need all 4 items + 10B Nomad to make Soulreaper Axe.");
			}
		}
...
//		UseItem.ItemonItem(c, itemUsed, useWith, itemUsedSlot, usedWithSlot);
		...
		if (itemUsed == 33383 && useWith == 33384 ||
				itemUsed == 33385 && useWith == 33384 ||
				itemUsed == 33384 && useWith == 33383||
				itemUsed == 33385 && useWith == 33383||
				itemUsed == 33384 && useWith == 33385) {
			if (c.getItems().getInventoryCount(33383) < 1 || c.getItems().getInventoryCount(33384) < 1 || c.getItems().getInventoryCount(33385) < 1) {
				c.sendMessage("You seem to be missing a cape!");
				return;
			}

			c.getItems().deleteItem2(33383, 1);
			c.getItems().deleteItem2(33384, 1);
			c.getItems().deleteItem2(33385, 1);
		...
		if (itemUsed == 21804 && useWith == 26269 || //Enhanced Demon x Crossbow
				itemUsed == 26269 && useWith == 21804) {

			c.getItems().deleteItem2(21804, 1);
			c.enhancerCrystal = true;
			c.sendErrorMessage("Your demon x crossbow will now perform double hits!");
		}
	...
	public static void handleItemOnNPC(Player player, NPC npc, int itemId, int slot) {
		...
		if (npcId == 10529) {
			if (itemId == 2399) {
				...
				if (Boundary.isIn(player, NomadVault.area)) {
					player.sendMessage("@red@You need to finish your current vault before starting another!");
					return;
				}

				player.NomadVaultObjects.clear();

				player.start(new DialogueBuilder(player).option("2% chance to obtain Ring Of Pursuit! \\n Which has a 5% chance to double nomad points when dissolving!", new DialogueOption("Enter Vault.", p -> {
					p.getPA().closeAllWindows();
					npc.forceChat("Good luck, " + p.getDisplayName() + " on your hunt for my treasure!");
					p.getPA().startTeleport(2067, 5403, 1, "foundry", false);
					p.sendMessage("@red@You hand nomad your key to access his secret storage vault!");
					p.sendMessage("@red@Click the final chest or teleport out to leave!");
					p.getItems().deleteItem2(2399, 1);
