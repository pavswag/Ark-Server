...
public class PvpWeapons {
    ...
    public PvpWeapons(Player player) {
        this.player = player;
        this.viggoraChainmaceCharges = 0;
        this.thammaronSceptreCharges = 0;
        this.crawsBowCharges = 0;
    }
     ...
     */
    public static boolean isPvpWeapon(int itemId) {
    ...
    public static boolean isPvpWeaponCharged(int itemId) {
        return Arrays.stream(PVP_WEAPONS).anyMatch(pvpWeaponId -> pvpWeaponId == itemId);
    }

    public static int UnchargeOnDeath(int itemId) {
        int weaponIdx = ArrayUtils.indexOf(PVP_WEAPONS, itemId);
        int unchargedWeapon = U_PVP_WEAPONS[weaponIdx];
        return unchargedWeapon;
    }
    ...
    public static boolean activateEffect(Player player, int weaponId) {
        return player.getItems().isWearingItem(weaponId, Player.playerWeapon)
                && player.getPvpWeapons().getCharges(weaponId) > 0 || weaponId == 27655 || weaponId == 27660 || weaponId == 27788;
    }
